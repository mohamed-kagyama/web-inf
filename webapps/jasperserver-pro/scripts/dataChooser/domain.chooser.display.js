...
dc.display = {
  ...
  getTreeItemsModelJSON: function getTreeItemsModelJSON() {
    return dc.display.convertNodesModelToJSON(dc.display.treeItemsModel);
  },
  getListItemsModelJSON: function getListItemsModelJSON() {
    return dc.display.convertNodesModelToJSON(dc.display.listItemsModel);
  },
  convertNodesModelToJSON: function convertNodesModelToJSON(rootNode) {
    var children = '';
  ...
  _prepareNodeForJSON: function _prepareNodeForJSON(node) {
    var nmodel = {};
    nmodel.id = node.id;
  ...
  _convertNodeToJSON: function _convertNodeToJSON(node) {
  ...
  init: function init(options) {
    ...
    this.initTreeEvents();
    domain.resetTreeSelectionHandler.init(this.MOVE_BUTTON_IDS.concat(['#' + this.ITEMS_TREE_DOM_ID]), function () {
      return [this.itemsTree];
    }.bind(this), this.itemSelectionClearCallback.bind(this));
    domain.registerClickHandlers([this.moveButtonsClickEventHandler.bind(this), this.displayAsButtonsClickEventHandler.bind(this)]);
    this.updateMoveButtonsState();
    this.itemsTree.renderTree();

    if (!this.flatList) {
      this.itemsTree.getRootNode() && this.itemsTree.getRootNode().expandAllChilds();
    }
  },
  setUpJSONModels: function setUpJSONModels(options) {
  ...
  buildMapModelFromNode: function buildMapModelFromNode(node, map) {
    map[node.id] = node; //It is not necessary to show tooltip on display page;
  ...
  buildTreeFromModel: function buildTreeFromModel(isFlat) {
    ...
    var rootObj = isFlat ? this.listItemsModel : this.treeItemsModel; // build items tree
    // build items tree

    this.itemsTree.setRootNode(this.itemsTree.processNode(rootObj)); //Set all nodes to be editable
  ...
  buildLeafArrayFromItemsMap: function buildLeafArrayFromItemsMap(itemsMap) {
    var leafArray = [];
  ...
  initFlatListOrderChanged: function initFlatListOrderChanged() {
    if (this.flatList) {
  ...
  initTree: function initTree() {
    ...
    [{
      rootNode: this.treeItemsModel,
      map: this.treeItemsMap
    }, {
      rootNode: this.listItemsModel,
      map: this.listItemsMap
  ...
  deselectAllExceptSiblings: function deselectAllExceptSiblings(node) {
  ...
  buildFlatLabel: function buildFlatLabel(node, isDisplayLabel) {
    if (!node) {
      return;
    }
    ...
    while (parentNode && parentNode != tree.getRootNode()) {
      ...
      if (flatLabel.indexOf(parentNode.name) == -1) {
        flatLabel = isDisplayLabel ? parentNode.name + '.' + flatLabel : parentNode.param.extra.label + '.' + flatLabel;
      }
  ...
  updateListItemsMapByNode: function updateListItemsMapByNode(node) {
    if (!node) {
      return;
    }
  ...
  nodeMouseup: function nodeMouseup(event) {
  ...
  nodeEdit: function nodeEdit(event) {
    var node = event.memo.node;
  ...
  updateNodeOrderNumberInItemsMap: function updateNodeOrderNumberInItemsMap(node, order) {
    if (this.flatList) {
      this.flatListOrderChanged = true;
  ...
  updateMoveButtonsState: function updateMoveButtonsState() {
  ...
  itemSelectionClearCallback: function itemSelectionClearCallback() {
  ...
  convertTreeModelToListModel: function convertTreeModelToListModel(node, lastFlatIndex, flatList) {
    if (!node) {
      return;
    }
  ...
  convertTreeViewToListView: function convertTreeViewToListView() {
    var newFlatModel = [];
  ...
  displayAsButtonsClickEventHandler: function displayAsButtonsClickEventHandler(element) {
    var eventHandled = false;
...
var dc_display = dc.display; // Default events factory for trees
