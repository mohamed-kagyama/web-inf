...
domain.ItemNode.addMethod('hide', function (hideChildren) {
  ...
  var disabledChildren = this.childs.findAll(function (child) {
    return child.disabled;
  }).length;
  var hiddenChildren = this.childs.findAll(function (child) {
  ...
  if (!this.disabled && visibleChildren === 0) {
    this._hidden = true;
    this.deselect();
    this.refreshStyle(); //    } else if (this.hasChilds() && disabledChildren > 0 && disabledChildren == visibleChildren) {
...
domain.ItemNode.addMethod('toJSON', function () {
  ...
  if (this.isParent()) {
    json.children = _.chain(this.childs).filter(function (c) {
      return !c.isHidden();
    }).invoke("toJSON").value();
  }
...
domain.TwoColumnNode.addMethod('_createNode', function () {
  ...
  this.refreshStyle(templH); // DOM element link on this tree node
  ...
  var origLabel = this.param.extra && this.param.extra.label ? this.param.extra.label : this.name;
  columnOne.insert(xssUtil.hardEscape(origLabel));
  var columnTwo = wrapper.childElements()[2];
  columnTwo.insert(xssUtil.hardEscape(this.name));
...
domain.TablesNode.addMethod('refreshStyle', function (element) {
  ...
  domain.ItemNode.prototype.refreshStyle.call(this, element); // Check item for validity
...
domain.TablesCustomNode = function (options) {
  ...
  this.nodeHeaderTemplateDomId = options.param.type === this.Types.Folder.name ? 'list_responsive_collapsible_type_tables:tables' : 'list_responsive_collapsible_type_tables:fields';
};

domain.TablesCustomNode.prototype = deepClone(domain.TablesNode.prototype); //Copy Move controller for domain desiger tables
...
domain.TablesCopyMoveController.addMethod('_handleDuplicatedNode', function (node, existingParent) {
  if (node.isParent()) {
    ...
    node.param.extra.originalId = node.param.extra.itemId; //TODO handle node.uri?
    ...
    node.param.extra.originalId = node.parent.param.extra.originalId + '.' + node.param.extra.itemId; //TODO handle node.uri?
  }
});
...
domain.DerivedTablesNode.prototype = deepClone(domain.TablesNode.prototype); // List for domain designer joins
...
domain.JoinsListItem.addMethod('refreshStyle', function () {
  ...
  if (this._getElement()) {
    this._hidden ? this._getElement().addClassName(layoutModule.HIDDEN_CLASS) : this._getElement().removeClassName(layoutModule.HIDDEN_CLASS);
  }
});
domain.JoinsListItem.addMethod('processTemplate', function (element) {
  ...
  columnOne.update(xssUtil.hardEscape(this._value.left.table.label + ':' + this._value.left.label));
  columnTwo.update(xssUtil.hardEscape(this._value.right.table.label + ':' + this._value.right.label));
...
domain.DisplayNode = function (options) {
  ...
  this.nodeHeaderTemplateDomId = 'list_responsive_collapsible_type_sets:sets'; //TODO: modify backend so error will be passed through extra.isInvalid field
...
domain.confirmDialog = {
  ...
  _init: function _init() {
    ...
    this._yesMessageHolder = $$(this._CONFIRM_DIALOG_OK_PATTERN).first(), this._noMessageHolder = $$(this._CONFIRM_DIALOG_CANCEL_PATTERN).first(), domain.registerClickHandlers([this._clickEventHandler.bind(this)]);
  },
  _show: function _show(templateDomId, onOk, onCancel, customEventHandler, mode) {
    this._onOk = onOk;
  ...
  _clickEventHandler: function _clickEventHandler(element) {
...
domain.detailsDialog = {
  ...
  _show: function _show(messageObj) {
  ...
  _createMessage: function _createMessage(messageObj) {
