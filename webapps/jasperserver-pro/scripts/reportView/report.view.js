...
var isValidJsonResponse = _runtime_dependenciesJrsUiSrcCoreCoreAjax.isValidJsonResponse;
...
var appendPostData = _runtime_dependenciesJrsUiSrcCoreCoreAjax.appendPostData;
...
function SaveAsDialog(inputOptions, saveHandler) {
  ...
  function updatePlaceToSave(place) {
    ...
    if (selNode) place.folder = selNode.param.uri;else place.folder = null;
  }
  ...
  this.open = function (placeToSave) {
    ...
    saveAsTree.observe('node:selected', function (event) {
      try {
        ...
        console && console.log('report.view[this.open] - ' + e);
      }
    });
...
Report.save = function () {
  ...
  var options = {
    ...
    callback: function callback(jsonResponse) {
      ...
      Report.savedState = window.jasperreports.reportviewertoolbar.currentState();
      ...
      switch (status) {
        ...
        case "confirmOverwrite":
          var dialog = new ConfirmationDialog({
            text: Report.getMessage(jsonResponse.response.msg)
          });
...
Report.replaceSingleOccurrence = function (text, value, replacement) {
  var firstIdx = text.indexOf(value); // only replace if the value occurs a single time
...
Report.saveAs = function () {
  if (Report.saveAsDialog === undefined) {
    Report.saveAsDialog = new SaveAsDialog({}
    /* use default options*/
    , function (location) {
      Report.doSaveAs(location, false); //TODO update container title: .reportViewFrame .content .header .title
...
Report.refreshSave = function () {
  ...
  success ? buttonManager.enable($('fileOptions')) : buttonManager.disable($('fileOptions'));
...
Report.confirmExit = function () {
  ...
  if (Report.savedState && Report.savedState != window.jasperreports.reportviewertoolbar.currentState() || Report.lastPageIndex != null && Report.snapshotSaveStatus == 'UPDATED') {
