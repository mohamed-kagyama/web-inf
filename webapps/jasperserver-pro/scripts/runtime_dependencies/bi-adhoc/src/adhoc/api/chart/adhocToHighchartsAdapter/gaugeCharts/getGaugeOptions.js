...
var RATIO_VERTICAL_CHART_UNITS = 10; // CIRCLE_CHART_SIZE is how much available space the will the circle of the chart take from the whole area for one chart

var CIRCLE_CHART_SIZE = 0.90; // in percents, it's 90%
// this variable specifies how thick the circle would be compared to the area for one chart

var CIRCLE_CHART_GAGE = 0.12; // in percents, it's 12%
...
var calculateSizes = function calculateSizes(options) {
  ...
  // So we need to get higher to the top of the circle. We can do this by using 'chartHorizontalSize / 2' which is the
  // radius of the circle.

  var fieldVerticalOffset = minimalContainerSideLength / 2 - circleDiameter / 2; // the radius values given in percents
  // and HC converts all percents to numbers by using the smallest container side, so do we

  var outerRadius = 100 * circleDiameter / minimalContainerSideLength; // the inner radius is smaller:

  var innerRadius = outerRadius * (1 - CIRCLE_CHART_GAGE);
  return {
...
var calculateFontData = function calculateFontData(options) {
  ...
  var widthAvailableForValueText = Math.floor(sizes.circleDiameter * (1 - CIRCLE_CHART_GAGE - 0.2));
  var heightAvailableForValueText = Math.floor(sizes.circleDiameter * 0.3);
  var widthAvailableForMeasure = Math.floor(sizes.circleDiameter * (1 - CIRCLE_CHART_GAGE - 0.2));
  var heightAvailableForMeasure = Math.floor(sizes.circleDiameter * 0.1);
  var texts = [];
  ...
  // Next, HC need to position this <text> element with two <tspan> inside a corresponding gauge circle, and it done by
  // taking the gauge circle point.
  ...
  // So, by having the center point of gauge circle and height of the <text> element HC can finally position the Value
  // and Measure above the gauge circle. the formula is simple:
  ...
  _.each(chartsData, function (chartData, paneIndex) {
    ...
    }); // we divide by 2 the height of Value and Measure because we'd like to shift them just by half, want to place
    // them in the middle


    correctOffset = Math.round(correctOffset / 2);
...
var getGaugeOptions = function getGaugeOptions(rowAxisLeafArray, columnAxisLeafArray, rowSlider, columnSlider, extraOptions) {
...
module.exports = getGaugeOptions;
