...
var AdhocDataProcessor = {
  ...
  fn: {
    ...
    load_common: function load_common() {
      var it = AdhocDataProcessor;
      ...
      if (it.metadata.axes.length) {
        ...
        it.measureLevelNumber = it.fn.getMeasureLevelNumber.call(it); // OLAP must go first because it may redraw the metadata.axes
        ...
        it.colSliderLevelCount = it.metadata.axes[1].length + 1 - (it.metadata.measureAxis === 1 ? 1 : 0); // we need sequential group mapping that skips over the measures level
    ...
    //
    getOLAPDimensionInfo: function getOLAPDimensionInfo() {
      var it = AdhocDataProcessor;
      var olapAxes = new Array(2); // each axis
      ...
      for (var i = 0; i < 2; i++) {
        ...
        if (dimMappingArray.length <= 0) {// empty axis  null array entry
        } else {
          ...
          for (var j = 0; j < dimMappingArray.length; j++) {
            ...
            if (hasMeasures && dimMapping.length > 0 && dimMapping[0].dimension === "Measures") continue; // Skip measure dimension.   Measures are invisible to the sliders

            var hasAll = false;
            if (dimensionAllMapping[j] === true) hasAll = true;
            var dim = {};
            dim.levels = []; // If there is an All level for a dimension we must include in the allowed
            // level groups
            ...
            currLevel = 1; // each level in this dimension
    ...
    getMeasureLevelNumber: function getMeasureLevelNumber(metadata) {
      if (!metadata) {
    ...
    //
    groupMapping: function groupMapping(axisIndex) {
      var it = AdhocDataProcessor;
      var i,
          groupMapping = [0]; //   top level 'All'
    ...
    //
    //
    dimensionMapping: function dimensionMapping(axisIndex) {
      var it = AdhocDataProcessor;
    ...
    createTreeNodeStructure: function createTreeNodeStructure(rootTreeNode) {
    ...
    connectParentNode: function connectParentNode(parent, childrenArray) {
      var i, child;
    ...
    getDataStyle: function getDataStyle() {
      var self = AdhocDataProcessor;

      if (self.metadata.axes[self.nonMeasureAxis].length === 0) {
        return dataStyleEnum.ON_SAME_AXIS_ONLY; // style 0   measures and groups on the same axis ONLY;
      } else if (self.metadata.axes[self.nonMeasureAxis].length > 0 && self.metadata.axes[self.measureAxis].length > 1) {
        return dataStyleEnum.ON_BOTH_AXIS; //  style 2   crosstab groups on both axes
      }

      return dataStyleEnum.ON_SEPARATE_AXIS; // style 1   measures on one axis and groups on the other
    },
    //
    ...
    //
    getNodeListForSliderLevel: function getNodeListForSliderLevel(axisIndex, sliderLevel) {
      var it = AdhocDataProcessor;
      ...
      if (AdhocDataProcessor.debug) {
        console.log("AdhocDataProcessor.getNodeListForSliderLevel: 4088h  nodeList for axisIndex=" + axisIndex + " sliderLevel=" + sliderLevel);
    ...
    //
    //
    getNodeListForDimLevelRadio: function getNodeListForDimLevelRadio(axisIndex, radioDims) {
      var it = AdhocDataProcessor;
      if (!it.metadata.isOLAP) throw "dataprocessor.getNodeListForDimLevelRadio called RadioButton method for non-OLAP query !";
      if (radioDims === null) throw "dataprocessor.getNodeListForDimLevelRadio.js  y5g59i8y  NULL radioLevel input !";
      var dims = it.dimensionMappings[axisIndex];
      if (!dims) throw "dataprocessor.getNodeListForDimLevelRadio.js 0oi098  no dimensionMappings for axis " + axisIndex;
      if (dims.length <= 0) return it.fn.getEmptyAxisResult();
      if (radioDims.length > dims.length) throw "dataprocessor.getNodeListForDimLevelRadio.js  98hu for axis: " + axisIndex + ", number of input dimensionss from radio buttons: " + radioDims.length + ", exceeds the number of dimensions in the chart data: " + dims.length; //
      ...
      if (radioDims.length + (axisIndex == it.metadata.measureAxis ? 1 : 0) < dims.length) {
        if (AdhocDataProcessor.debug) {
          console.log("AdhocDataProcessor.getNodeListForDimLevelRadio.js " + "need to add  Radio Dimensions padding");
      ...
      var dimLevels = []; // convert the 'radio button' settings to an internal Dimension Level structure
      //
      ...
      if (measuresFirst && radioDims.length === 0) {
        dimLevels.push({
          startLevel: 0,
          // startLevel == treeNode level immediately BEFORE our Dimension
          endLevel: 1,
          // endLevel == slider position, for Dimension totals == startLevel
        ...
        for (var rDim = 0; rDim < radioDims.length; rDim++) {
          ...
          dimCount++; // if we have not already handled measures, always check ahead to see if the next
          ...
          if (it.metadata.measureAxis === axisIndex) {
            if (measuresOffSet <= 0) {
              ...
              if (rDim + 2 > dims.length) {
                throw "dataprocessor.getNodeListForDimLevelRadio.js  43098h  we were expecting dimensions to be at least " + (rDim + 2) + " in length, instead it is " + dims.length;
      ...
      if (AdhocDataProcessor.debug) {
        console.log("AdhocDataProcessor.getNodeListForDimLevelRadio: 39uh  nodeList for axis=" + axisIndex + " radioDims, dimLevels ");
    ...
    //
    getNodeListForDimLevels: function getNodeListForDimLevels(axisIndex, dimLevels) {
      var it = AdhocDataProcessor;
    ...
    //
    getNodeListForSliderLevelGroupMeasure: function getNodeListForSliderLevelGroupMeasure(axisIndex, sliderLevel) {
      var it = AdhocDataProcessor;
      var totalLevels = it.metadata.axes[axisIndex].length;
    ...
    getEmptyAxisResult: function getEmptyAxisResult() {
      var it = AdhocDataProcessor;
    ...
    //
    //
    getNodeListForDimensionLevelsMeasure: function getNodeListForDimensionLevelsMeasure(axisIndex, dimLevels) {
      var it = AdhocDataProcessor;
      var isMeasureAxis = false;
      if (axisIndex === it.metadata.measureAxis) isMeasureAxis = true;
      var groupMapping = axisIndex === 0 ? it.rowGroupMapping : it.colGroupMapping; //    if   the measures are not at the leaf level
      ...
      for (var dimIndex = 0; dimIndex < dimLevels.length; dimIndex++) {
        ...
        if (AdhocDataProcessor.debug) {
          console.log("AdhocDataProcessor.getNodeListForDimensionLevelsMeasure: 93287h  begin qualifying nodes for " + "dim=" + dimIndex + " startlevel=" + startLevel + ", endLevel=" + endLevel + "localLeafLevel=" + localLeafLevel);
        }

        var isLeafLevel = false; // we can only have a leaf level query as a possibility if:
        // for non-OLAP
        ...
        if (!it.metadata.isOLAP) {
          if (dimIndex >= dimLevels.length - 1) {
            if (axisIndex === 0) {
              if (endLevel >= it.rowSliderLevelCount - 1) isLeafLevel = true;
            } else {
              if (endLevel >= it.colSliderLevelCount - 1) isLeafLevel = true;
            }
          }
        } else {
        ...
        if (isLeafLevel) {
          ...
          if (AdhocDataProcessor.debug) {
            console.log("AdhocDataProcessor.getNodeListForDimensionLevelsMeasure: we9iuhf  LeafLevel nodeList for targetLevel=" + targetLevel);
            console.log(nodeList);
          } // 2 leaf level cases:
          ...
          var filteredNodeList = []; //  this is the level limit beyond which we stop recursively
          ...
          for (var i = 0; i < nodeList.length; i++) {
            ...
            while (parentNode != null) {
              ...
              if (parentNode.isAll === true) break;
          ...
          //  processing for NOT isLeafLevel
          ...
          for (var i = 0; i < nodeList.length; i++) {
            var node = nodeList[i];
            var recurse = false; // note: if we pass the level > 0 test, then we are guaranteed to have a parent
    ...
    //
    getNodeListForActualLevel: function getNodeListForActualLevel(axisIndex, children, targetLevel, nodeList) {
      var it = AdhocDataProcessor;
    ...
    //
    chaseAllNodesToLeafToList: function chaseAllNodesToLeafToList(topParent, node, leafLevelNumber, addToArray) {
      var it = AdhocDataProcessor;
        ...
        if ('children' in node) {
          if (node.children.length > 0) {
            for (var i = 0; i < node.children.length; i++) {
              ...
              var recurse = false; // ALL measure levels allowed through
    ...
    recurseLeafArrayFromSimpleNode: function recurseLeafArrayFromSimpleNode(simpleNode, result) {
      var it = AdhocDataProcessor;
    ...
    getDataFromRowColumn: function getDataFromRowColumn(rowLeaf, columnLeaf) {
      ...
      if (columnLeaf.axisCoordinate < 0) throw "dataprocessor.getDataFromRowColumn   column Coordinate < 0 !"; //if (AdhocDataProcessor.debug) {
    ...
    //
    getLabelNameArray: function getLabelNameArray(axisIndex, leafNode) {
      var it = AdhocDataProcessor;
      var result = [];
      if (leafNode === null || _.isUndefined(leafNode)) return result;
      ...
      if (it.metadata.isOLAP) {
        ...
        var dimLevels = it.dimensionLevels[axisIndex]; // starting at root  go down the dimensions until we find the dimension that brackets our level
    ...
    isMeasuresLastOnAxis: function isMeasuresLastOnAxis(axisIndex) {
      var it = AdhocDataProcessor;
    ...
    doPathsMatch: function doPathsMatch(left, right) {
      while (left.parent) {
    ...
    getLevelsWithoutMeasures: function getLevelsWithoutMeasures(levels) {
      return _.chain(levels).filter(function (level) {
        return level.dimension != "Measures";
      }).map(function (level) {
        level.name = xssUtil.unescape(level.name);
        level.label = xssUtil.unescape(level.label);
        return level;
      }).value();
    },
    getLevelIndex: function getLevelIndex(levels, level) {
      for (var i = 0; i < levels.length; i++) {
    ...
    levelsToLevelNumbers: function levelsToLevelNumbers(levels, axis) {
    ...
    _getOlapLevelNumbers: function _getOlapLevelNumbers(selectedLevels, axisDimensionsInfo) {
      var me = AdhocDataProcessor.fn;
      return _.map(selectedLevels, function (level) {
        ...
        return {
          level: me._getOlapDataProcessorLevelNumber(level, dimension)
        };
      });
    },
    _getOlapDataProcessorLevelNumber: function _getOlapDataProcessorLevelNumber(level, dimension) {
    ...
    _getNonOlapDataProcessorLevelNumber: function _getNonOlapDataProcessorLevelNumber(selectedLevels, allLevels) {
      var me = AdhocDataProcessor.fn;

      if (selectedLevels.length === 0) {
        return 0;
      } else {
        var levels = me.getLevelsWithoutMeasures(allLevels); // The slider position will be level index + 1 because of grand total on the position 0.
...
module.exports = {
  ...
  getColumnSliderLevelCount: function getColumnSliderLevelCount() {
    return AdhocDataProcessor.colSliderLevelCount;
  },
  getRowSliderLevelCount: function getRowSliderLevelCount() {
