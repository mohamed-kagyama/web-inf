...
var tableConditionCollectionTemplate = require("text!../template/tableConditionCollectionTemplate.htm");

var TableConditionView = require('./TableConditionView');
...
module.exports = Epoxy.View.extend({
  ...
  el: function el() {
    return _.template(tableConditionCollectionTemplate, {
      i18n: i18n
    });
  },
  ...
  addModel: function addModel(modelJSON, skipRegistration) {
    ...
    if (applyTo === 'detailrows') {
      ...
      dataType = this.columnComponentModel.parent.columnGroups.findWhere({
        id: applyTo
  ...
  addAll: function addAll(arrJsonModels) {
    var self = this;
  ...
  addNewCondition: function addNewCondition(evt) {
    if (this.$(evt.target).is('.disabled')) {
      return;
    }

    var tableConditionModel = new TableConditionModel(),
        model = this.addModel(tableConditionModel.defaults()),
        view = this.getSubviewByModel(model); // preselect operator
    // preselect operator
