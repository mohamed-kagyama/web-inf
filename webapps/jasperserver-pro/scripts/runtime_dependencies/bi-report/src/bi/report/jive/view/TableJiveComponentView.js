...
var ColumnResizeMarkerView = require('./overlay/ColumnResizeMarkerView');

var TableFilterDialog = require('../jr/dialogs/TableFilterDialog');

var TableFormatDialog = require('../jr/dialogs/TableFormatDialog');

var AlertDialog = require("runtime_dependencies/js-sdk/src/common/component/dialog/AlertDialog");

var showColumnOptionPredicate = require('./predicate/showColumnOptionPredicate');

var hideColumnOptionPredicate = require('./predicate/hideColumnOptionPredicate');

var jiveActions = require('../enum/jiveActions');

var $ = require('jquery');

var dragLabelTemplate = require("text!./overlay/template/dragLabelTemplate.htm");
 ...
 * @version: $Id$
 */
var log = logger.register("TableJiveComponentView"); //needed for proper rendering of calendar in JIVE
...
function computeDropBoundariesForColumnMove() {
  ...
  for (i = 0; i < tableColumns.length; i++) {
    ...
    colData = _.findWhere(this.model.config.allColumnsData, {
      uuid: column.uuid
    });
...
function onOverlayDragStart(evt, ui) {
  this.currentColMoveData = _.findWhere(this.visibleColumnsMoveData, {
    uuid: this.currentColumn.get("id")
  });
...
function onOverlayDrag(evt, ui) {
  var i,
      ln = this.visibleColumnsMoveData.length,
      colMoveData,
      refColMiddle,
      isLeftToRight,
      ...
      hoverCol; // determine move direction relative to selected column
  ...
  for (i = 0; i < ln; i++) {
    ...
    if (pageX <= colMoveData.right) {
      ...
      if (pageX <= refColMiddle) {
        ...
        this.resizeMarker.$el.offset({
          left: colMoveData.left - scrollAmount
        });
        ...
        this.resizeMarker.$el.offset({
          left: colMoveData.right - scrollAmount
        });
...
module.exports = BaseJiveComponentView.extend({
  ...
  initTableEvents: function initTableEvents() {
    ...
    this.tableElement.on("click touchend", ".jrPage td.jrcolHeader.interactiveElement[data-tableuuid=" + tableID + "]", _.bind(onTableClick, this));
    this.tableElement.on("click touchend", ".jrPage td.jrcel[data-tableuuid=" + tableID + "]", _.bind(onTableClick, this));
    ...
    if (this.stateModel.isFloatingTableHeaderEnabled() && this.model.get("hasFloatingHeader")) {
      this.$scrollContainer = $(this.stateModel.get("container")); // relax the scroll event triggering a bit by throttling it

      this.$scrollContainer.on("scroll", _.throttle(_.bind(this._scrollHeader, this), 100, {
        leading: true,
        trailing: true
      }));
      this.scrollData = {
  ...
  initJiveComponents: function initJiveComponents(reportContainer) {
    ...
    this.overlay.$el.draggable({
      ...
      helper: function helper(evt) {
        return $(_.template(dragLabelTemplate, {
          i18n: i18n
        })).show();
      }
    });
  ...
  sort: function sort(actionModel) {
    var order = actionModel.get("order");

    if (order) {
      this.currentColumn.sort({
        order: order
      });
    }
  },
  filter: function filter(actionModel, actionView) {
    this.filterDialog.open(this.currentColumn);
  },
  format: function format(actionModel, actionView) {
    this.formatDialog.open(this.currentColumn);
  },
  hideColumn: function hideColumn(actionModel) {
    this.currentColumn.hide();
  },
  showColumn: function showColumn(actionModel) {
    var index = actionModel.get("index");
    this.currentColumn.unhide(_.isArray(index) ? index : [index]);
  },
  setGenericProperties: function setGenericProperties(properties) {
    this.genericProperties = properties;
  },
  getGenericProperties: function getGenericProperties() {
    return this.genericProperties;
  },
  getHoverMenuChildren: function getHoverMenuChildren() {
  ...
  getColumnData: function getColumnData($target) {
  ...
  setCurrentColumn: function setCurrentColumn(columnID) {
    this.currentColumn = _.find(this.model.columns, function (column) {
      return column && column.get("id") === columnID;
    });
  },
  selectColumn: function selectColumn(columnID, columnData) {
  ...
  _scrollHeader: function _scrollHeader(forceScroll) {
    var scrollContainer = this.$scrollContainer,
        scrolledTop = false,
        scrolledLeft = false,
        scrollData = this.scrollData; // Determine scroll direction and value
    ...
    var floatingTable = this._getHeaderTable(firstHeader),
        ...
        diff = lastTableCel.length ? lastTableCel.offset().top - floatingTable.outerHeight() - containerTop : -1,
  ...
  _getHeaderTable: function _getHeaderTable(firstHeader) {
  ...
  _setToolbarPositionWhenFloating: function _setToolbarPositionWhenFloating(floatingTable) {
  ...
  _setToolbarDefaultPosition: function _setToolbarDefaultPosition() {
