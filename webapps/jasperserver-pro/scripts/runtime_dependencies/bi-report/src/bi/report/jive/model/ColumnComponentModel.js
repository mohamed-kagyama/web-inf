 ...
 */
var localLogger = logger.register("ColumnComponentModel");
  ...
  if (this.get('canFilter')) {
    ...
    if (_.isUndefined(this.get('filterValue')) || _.isNull(this.get('filterValue')) || filterValueInBetweenIsUnderfinedOrNull) {
      var fieldValueStart = this.get('filtering') ? this.get('filtering').filterData.fieldValueStart : undefined,
          fieldValueEnd = this.get('filtering') ? this.get('filtering').filterData.fieldValueEnd : undefined; // TODO: optimize this
      ...
      if (jiveDataConverter.dataTypeToSchemaFormat[this.get('dataType')] === 'datetime') {
        if ((this.get('filterOperator') === 'between' || this.get('filterOperator') === 'not_between') && fieldValueStart != null && fieldValueEnd != null) {
          this.set('filterValue', [jiveDataConverter.jQueryUiTimestampToIsoTimestamp(fieldValueStart, this.parent && this.parent.config ? this.parent.config.genericProperties : undefined), jiveDataConverter.jQueryUiTimestampToIsoTimestamp(fieldValueEnd, this.parent && this.parent.config ? this.parent.config.genericProperties : undefined)], {
            silent: true
          });
        } else if (fieldValueStart != null) {
          this.set('filterValue', jiveDataConverter.jQueryUiTimestampToIsoTimestamp(fieldValueStart, this.parent && this.parent.config ? this.parent.config.genericProperties : undefined), {
            silent: true
          });
        }
      } else if (jiveDataConverter.dataTypeToSchemaFormat[this.get('dataType')] === 'time') {
        if ((this.get('filterOperator') === 'between' || this.get('filterOperator') === 'not_between') && fieldValueStart != null && fieldValueEnd != null) {
          this.set('filterValue', [jiveDataConverter.jQueryUiTimeToIsoTime(fieldValueStart, this.parent && this.parent.config ? this.parent.config.genericProperties : undefined), jiveDataConverter.jQueryUiTimeToIsoTime(fieldValueEnd, this.parent && this.parent.config ? this.parent.config.genericProperties : undefined)], {
            silent: true
          });
        } else if (fieldValueStart != null) {
          this.set('filterValue', jiveDataConverter.jQueryUiTimeToIsoTime(fieldValueStart, this.parent && this.parent.config ? this.parent.config.genericProperties : undefined), {
            silent: true
          });
        }
      }
...
module.exports = BaseComponentModel.extend({
  ...
  parse: function parse(response) {
    ...
    if (response.valuesTabContent) {
      ...
      this.detailsRowFormat.set(this.detailsRowFormat.parse(response.valuesTabContent), {
        silent: true
      });
    }
  ...
  actions: {
    'change:sortOrder': function changeSortOrder() {
    ...
    'change:filter': function changeFilter() {
    ...
    'change:detailsRowFormat': function changeDetailsRowFormat() {
      return {
        'actionName': 'editTextElement',
        'editTextElementData': {
          'applyTo': 'detailrows',
  ...
  toReportComponentObject: function toReportComponentObject() {
    ...
    if (columns) {
      var thisColumnProps = _.findWhere(columns, {
        uuid: this.get('id')
      });
