...
var headerToolbarViewFactory = require('../factory/headerToolbarViewFactory');

var JiveOverlayView = require('./overlay/JiveOverlayView');
...
module.exports = BaseJiveComponentView.extend({
  ...
  initJiveEvents: function initJiveEvents(crosstab) {
    ...
    this.crosstabElement.on('click touchend', 'td.jrxtcolheader[data-jrxtid=\'' + fragmentId + '\']', _.bind(this._onClick, this, crosstab));
    this.crosstabElement.on('click touchend', 'td.jrxtdatacell[data-jrxtid=\'' + fragmentId + '\']', _.bind(this._onClick, this, crosstab));
    this.crosstabElement.on('click touchend', 'td.jrxtrowheader[data-jrxtid=\'' + fragmentId + '\']', _.bind(this._onClick, this, crosstab));
  ...
  initJiveComponents: function initJiveComponents(reportContainer) {
    //TODO i18n
  ...
  _onClick: function _onClick(crosstab, e) {
  ...
  _onHeaderToolbarSelect: function _onHeaderToolbarSelect(buttonView, buttonModel, e) {
    var order = buttonModel.get("order");
    buttonView.$el.hasClass("disabled") ? this.hide() : this.sort(order);
  },
  selectDataColumn: function selectDataColumn(crosstab, cell) {
  ...
  selectRowGroup: function selectRowGroup(crosstab, cell) {
    var columnIdx = cell.data('jrxtcolidx');
    var fragmentId = cell.data('jrxtid');
  ...
  getOverlaySize: function getOverlaySize(lastEl, firstEl, scaleFactor) {
  ...
  resetSelected: function resetSelected(selected) {
    this.selected = selected ? selected : null;
  },
  _getModulesToLoad: function _getModulesToLoad() {
    var modules = BaseJiveComponentView.prototype._getModulesToLoad.call(this); //TODO: find out why it's not in a themes folder
  ...
  _initFloatingHeaders: function _initFloatingHeaders() {
    this.$scrollContainer = $(this.stateModel.get("container")); // scrollContainer must have position!=static in order for the floating headers to behave properly

    if (this.$scrollContainer.css("position") === "static") {
      this.$scrollContainer.css("position", "relative");
    }

    this.canFloat = false; // relax the scroll event triggering a bit by throttling it

    this.$scrollContainer.on("scroll", _.throttle(_.bind(this._scrollHeaders, this), 100, {
      leading: true,
      trailing: true
    }));
    this.scrollData = {
  ...
  _scrollHeaders: function _scrollHeaders(evt, forceScroll) {
    ...
    var scrollContainer = this.$scrollContainer,
        scrolledTop = false,
        scrolledLeft = false,
        scrollData = this.scrollData; // Determine scroll direction

    if (scrollData.scrollTop != null) {
   ...
   * @private
   */
  _isCrosstabVisible: function _isCrosstabVisible() {
    var scrollContainer = this.$scrollContainer,
  ...
  _scrollColumnHeader: function _scrollColumnHeader() {
    var scrollContainer = this.$scrollContainer,
  ...
  _scrollRowHeader: function _scrollRowHeader() {
    var scrollContainer = this.$scrollContainer,
        scrollData = this.scrollData,
        crosstabId = this.model.getFragmentId();
    ...
    var floatingTbl = this.floatingRowHeader,
        ...
        diff = lastTableCel.length ? lastTableCel.offset().left - floatingTbl.width() - containerLeft : -1,
        // if last cell is not visible, hide the floating header
    scaleFactor = this.model.get("scaleFactor"),
        floatCondition = headerLeft - containerLeft < 0 && diff > 0;

    if ((!scrollData.bRowMoved && floatCondition || scrollData.bRowMoved && floatCondition) && 0.8 * scrollContainer.outerWidth() > floatingTbl.outerWidth() * scaleFactor) {
      floatingTbl.show();
  ...
  _scrollCrossSection: function _scrollCrossSection() {
    var scrollContainer = this.$scrollContainer,
        scrollData = this.scrollData,
        crosstabId = this.model.getFragmentId();
    ...
    if ((scrollData.bColMoved || scrollData.bRowMoved) && 0.8 * scrollContainer.outerWidth() > floatingRowTbl.outerWidth() * scaleFactor) {
  ...
  _getFloatingTable: function _getFloatingTable(firstHeader, crosstabId, tableClass, elementClass, altElementClass) {
    ...
    if (firstHeader && tbl.length === 0) {
        ...
        tbl.on("click", ".jrxtrowfloating", function (evt) {
          ...
          if (!$(evt.target).parent().is("._jrHyperLink")) {
            var jo = $(this),
                ...
                altJo = tbl.parent().find("table.jrPage td.jrxtrowfloating[data-jrxtid='" + crosstabId + "']").filter("td[data-jrxtcolidx='" + colIdx + "']").eq(0);
      ...
      var parentTable = firstHeader.closest("table"),
          ...
          startIndex = 0,
          colSpanLength = 0,
          finishedCalculatedStartIndex = false;
      ...
      if (firstHeader.length > 0) {
        ...
        if (row.get(0) === lastRow.get(0)) {
          // need to compare the actual dom nodes
          rows.push(row);
        } else {
        ...
        $.each(rows, function (idx, row) {
          ...
          clone.attr("valign", $row.attr("valign")); // add only the tds with elementClass class
          ...
          for (i = 0; i < rowTDs.length; i++) {
            ...
            if (rowTD.data("jrxtid") === crosstabId && (rowTD.is("." + elementClass) || altElementClass && rowTD.is("." + altElementClass))) {
              ...
              if (elementClass == "jrxtrowfloating" && rowTD.is(".jrxtinteractive")) {
                ...
                cloneTD = rowTD.clone(); // Fix for bug #41786 - set width/height with css method to take box-sizing into account
  ...
  _getColumnHeaderFloatingTable: function _getColumnHeaderFloatingTable(firstHeader, crosstabId) {
      ...
      tbl.on("click", ".jrxtcolfloating", function (evt) {
        ...
        if (!$(evt.target).parent().is("._jrHyperLink")) {
          var jo = $(this),
              crosstabId = jo.attr("data-jrxtid"),
              colIdx = jo.attr("data-jrxtcolidx"),
              crosstabFloatingHeader = tbl.parent().find("table.jrPage td.jrxtcolheader[data-jrxtid='" + crosstabId + "']").filter("td[data-jrxtcolidx='" + colIdx + "']").eq(0);
  ...
  _markCrossHeaderElements: function _markCrossHeaderElements(crosstabId) {
    var scrollContainer = this.$scrollContainer; // Prepare the crosssection in case it doesn't exist
  ...
  _onColumnHeaderScroll: function _onColumnHeaderScroll() {
    if (this.headerToolbar.$el.is(":visible")) {
      var floatingTable, floatingHeader, my; // data column is selected
      ...
      if (this.selected.isColumn) {
        ...
        floatingHeader = floatingTable.find("td.jrxtcolheader[data-jrxtcolidx='" + this.selected.header.data("jrxtcolidx") + "']").first();
          ...
          floatingTable = this.floatingCrossHeader; // the row header is floating and no cross-header table present
      ...
      */
          ...
          this.headerToolbar.$el.offset({
            top: newTop,
            left: this.headerToolbar.$el.offset().left
          });
        }
    }
  },
  _onColumnHeaderScrollStop: function _onColumnHeaderScrollStop() {
    if (this.headerToolbar.$el.is(":visible")) {
      this._setToolbarAndOverlayPosition();
    }
  },
  _onRowHeaderScroll: function _onRowHeaderScroll() {
    ...
    var floatingRowTable = this.floatingRowHeader,
        scaleFactor = this.model.get("scaleFactor"),
        floatingRowTableRight = floatingRowTable.offset().left + floatingRowTable.width() * scaleFactor; // toolbar is visible and selected header goes under the floating row header
  ...
  _onRowHeaderScrollStop: function _onRowHeaderScrollStop() {
    if (!this.isSelectionActive) {
      return;
    }
  ...
  _setToolbarAndOverlayPosition: function _setToolbarAndOverlayPosition() {
    ...
    if (scrollData.bColMoved) {
      ...
      if (this.selected.isColumn) {
        ...
        tableHeader = floatingTable.find("td.jrxtcolheader[data-jrxtcolidx='" + this.selected.header.data("jrxtcolidx") + "']").first();
          ...
          floatingTable = this.floatingCrossHeader || this._getFloatingTable(null, crosstabId, "jr_floating_cross_header"); // the row header is floating and (no cross-header table present or present but with no row headers)
       ...
       */
          ...
          this.headerToolbar.$el.offset({
            top: newTop,
            left: this.selected.header.offset().left
          });
          bHeaderToolbarPositioned = true;
        }
    }
