...
var biComponentErrorCodes = require("runtime_dependencies/js-sdk/src/common/bi/error/enum/biComponentErrorCodes");
...
var ReportPropertiesModel = require('./model/ReportPropertiesModel');

var reportOutputFormats = require('./enum/reportOutputFormats');

var interactiveComponentTypes = require('./jive/enum/interactiveComponentTypes');

var reportEvents = require('./enum/reportEvents');

var logger = require("runtime_dependencies/js-sdk/src/common/logging/logger");

var reportSchema = require("json!./schema/Report.json");

var reportExportSchema = require("json!./schema/ReportExport.json");

var reportSearchSchema = require("json!./schema/ReportSearch.json");

var reportSaveSchema = require("json!./schema/ReportSave.json");

var chartSchema = require("json!./schema/Chart.json");

var crosstabColumnSchema = require("json!./schema/CrosstabDataColumn.json");

var crosstabRowSchema = require("json!./schema/CrosstabRowGroup.json");

var tableColumnSchema = require("json!./schema/TableColumn.json");

/*
 * Copyright (C) 2005 - 2020 TIBCO Software Inc. All rights reserved. Confidentiality & Proprietary.
 * Licensed pursuant to commercial TIBCO End User License Agreement.
 */
var localLogger = logger.register("Report");
...
function run(dfd, instanceData, controller, refresh, stateModel) {
  ...
  var pages = _.isObject(instanceData.properties.pages) ? instanceData.properties.pages.pages : instanceData.properties.pages,
      anchor = _.isObject(instanceData.properties.pages) ? instanceData.properties.pages.anchor : undefined;
  ...
  if (instanceData.properties.reportLocale) {
    controller.model.execution.set({
      reportLocale: instanceData.properties.reportLocale
    });
  }

  if (instanceData.properties.reportTimeZone) {
    controller.model.execution.set({
      reportTimeZone: instanceData.properties.reportTimeZone
    });
  }
  ...
  var tryUpdateComponents = function tryUpdateComponents() {
    var dfd = $.Deferred(),
        err;
    ...
    var componentId = controller.updateComponent && controller.updateComponent.componentId,
        ...
        componentToUpdate = !_.isUndefined(componentId) && _.findWhere(controller.components.getComponents(), {
      name: componentId
    }) || _.findWhere(controller.components.getComponents(), {
      id: componentId
    });
    ...
    if (validationResult) {
      ...
      if (actions && _.isArray(actions) && actions.length) {
        ...
        controller.runReportAction(actions).done(function () {
          dfd.resolve(_.findWhere(controller.components.getComponents(), {
            name: componentId
          }));
        }).fail(function (error) {
          if (error.source === 'export' || error.source === 'execution') {
            ...
            if (_.include([biComponentErrorCodes.REPORT_EXECUTION_FAILED, biComponentErrorCodes.REPORT_EXECUTION_CANCELLED, biComponentErrorCodes.REPORT_EXPORT_FAILED, biComponentErrorCodes.REPORT_EXPORT_CANCELLED], err)) {
              localLogger.error(err.toString());
            } else {
  ...
  var tryRenderReport = function tryRenderReport() {
  ...
  var onSuccessfulRun = function onSuccessfulRun() {
  ...
  var onFailedRun = function onFailedRun(error) {
    var err;
    controller.view.hideOverlay();
    parametersChanged && controller.model.execution.set({
    ...
    tryUpdateComponents().done(function () {
      if (parametersChanged || refresh) {
        ...
        if (locationChanged || searchActionExecutedSuccessfuly || controller.updateComponent) {
          controller.fetchPageHtmlExportAndJiveComponents().then(onSuccessfulRun, onFailedRun);
        } else {
    ...
    if (!stateModel.get('_destroyed')) {
      try {
        ...
        self.run().always(function () {
          ...
          dfd.resolve(!_.isUndefined(componentId) && _.findWhere(controller.components.getComponents(), {
            name: componentId
          }) || _.findWhere(controller.components.getComponents(), {
            id: componentId
          }));
...
var Report = function Report(properties) {
  ...
  var controller = new ReportController(stateModel),
      eventManager = _.extend({}, Backbone.Events); // hack to prevent CSS overrides
