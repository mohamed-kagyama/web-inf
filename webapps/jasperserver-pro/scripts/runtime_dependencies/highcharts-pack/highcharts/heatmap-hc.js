...
///////////////////////////////////////////////////////////////////////
//
//  this file was renamed from heatmap.js to prevent blocking by Ad Blockers
//
// #1 amd-fication
//
// #2 7/16/14, 7/17/14, 7/29/2014 (also JRS-17940):
//      Extending heatmap to be able to render chart by canvas instead of SVG because canvas
//      can handle big amount of data while SVG requires a lot of DOM node (one per each cell)
//      and it slows down a browser a lot.
//      This patch also contains a code which adds a CSS styling to the tooltip in case of canvas rendering
//
///////////////////////////////////////////////////////////////////////
'use strict';
//JASPERSOFT #1
(function (factory, globalScope) {
    ...
    if (typeof module === 'object' && module.exports) {
        module.exports = factory(require("highcharts"), require("jquery"));
    } else if (typeof define === "function" && define.amd) {
        define(["highcharts", "jquery"], factory);
    } else {
        factory(globalScope.Highcharts, globalScope.jQuery);
    }
//END JASPERSOFT #1
}(function (Highcharts, jQuery) {
	(function (H) {
		/**
    ...
    var
        renderingByCanvas = false;

    Highcharts.wrap(Highcharts.seriesTypes["heatmap"].prototype, "drawPoints", function () {

        // We are modifying the code of this function to use canvas instead of svg in most cases
        ...
        renderingByCanvas = false; // by default we render in SVG (just like the original code did)

        // heatmap module may also be used to render "heatmap time series" chart
        // and in this case we need to use canvas
        if (chart.options.chart.isHeatMapTimeSeriesChart) {
            renderingByCanvas = true;
        }

        // for export case we need to use SVG in anyway
        if (chart.renderer.forExport) {
            renderingByCanvas = false;
        }

        if (renderingByCanvas) {

            if (ctx = series.getContext()) {
                ctx.canvas.height = chart.renderer.plotBox.height;
                ctx.canvas.width = chart.renderer.plotBox.width;
                ...
                series.addStylesForTooltipOnCanvas();

            } else {
                chart.showLoading("Your browser doesn't support HTML5 canvas, <br>please use a modern browser");
            ...
            // Here we run the original function drawPoints()
    ...
    // Highcharts.Pointer.runPointActions reacts on some events, like mouseover, pinch, touch, etc
    // and we need to wrap it to know if user hovers some point on canvas
    Highcharts.wrap(Highcharts.Pointer.prototype, 'runPointActions', function (proceed, e, p) {
        var
            chart;

        // call the wrapped function: "e" and "p" are parameters from the wrapped function
        this.proceed(e, p);

        if (!renderingByCanvas) {
            // don't do anything unless we were rendered by canvas
            return;
        }

        chart = this.chart;

        if (e._syntheticEvent === true) {
            // don't handle event which we have created
            return;
        }
        ...
        Highcharts.each(chart.series, function (series) {
            ...
            // mark this event with flag to what we have launched it
            // to avoid handling it again
            e._syntheticEvent = true;
            point.onMouseOver(e);
        });
    });

    Highcharts.extend(Highcharts.seriesTypes["heatmap"].prototype, {
        ...
        addStylesForTooltipOnCanvas: function () {
            ...
            if (renderingByCanvas) {
                ...
                // remove all styles if we were switched to other rendering method
