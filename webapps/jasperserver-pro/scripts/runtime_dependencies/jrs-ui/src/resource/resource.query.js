...
var DataSourceController = require('../dataSource/DataSourceController');
...
var resourceQuery = {
      ...
      this._newDataSourceLink.observe('click', function (e) {
        if (self._localDataSource.checked) {
          ...
          options.saveFn = function (dataSource, model) {
            ...
            input.value = JSON.stringify(_.extend(model.toJSON(), {
  ...
  _isDataValid: function _isDataValid() {
    var res = true;
    if (this._label) res = res && ValidationModule.validate(resource.getValidationEntries([this._label]));
    ...
    if (this._description) res = res && ValidationModule.validate(resource.getValidationEntries([this._description])); // When using Prototype JS to disable element its sets disabled property to appropriate state.
    ...
    if (this._dataSourceUri && !this._dataSourceUri.disabled) res = res && ValidationModule.validate(resource.getValidationEntries([this._dataSourceUri]));
    if (this._query) res = res && ValidationModule.validate(resource.getValidationEntries([this._query]));
    return res;
  },
  jumpTo: function jumpTo(pageTo) {
...
if (typeof require === 'undefined') {
  document.observe('dom:loaded', function () {
    resourceQuery.initialize(window.localContext.initOptions);
  });
}
