...
var isIE10 = _utilUtilsCommon.isIE10;
...
var resourceReport = {
  ...
  _filePathValidator: function _filePathValidator(value) {
    var isValid = true;
    var errorMessage = '';
    ...
    if (isValid) ValidationModule.hideError($('fileName'));else ValidationModule.showError($('fileName'), errorMessage);
    return {
      isValid: isValid,
      errorMessage: errorMessage
    };
  },
  _resourceUriValidator: function _resourceUriValidator(value) {
    var isValid = true;
    var errorMessage = '';
  ...
  _adjustFileSelectorPosition: function _adjustFileSelectorPosition() {
  ...
  editResource: function editResource(resourceName) {
  ...
  editControl: function editControl(resourceName) {
    $(this.RESOURCE_NAME_ID).setValue(resourceName);
    $(this.EDIT_CONTROL_BUTTON_ID).click();
  },
  removeControl: function removeControl(resourceName) {
    $(this.RESOURCE_NAME_ID).setValue(resourceName);
...
if (typeof require === "undefined") {
  document.observe("dom:loaded", function () {
    resourceReport.initialize(window.localContext.initOptions);
  });
}
