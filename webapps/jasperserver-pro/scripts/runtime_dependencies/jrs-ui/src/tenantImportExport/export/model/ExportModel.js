...
var i18n = require("bundle!ImportExportValidationBundle");

var BaseModel = require("runtime_dependencies/js-sdk/src/common/model/BaseModel");

var ResourcesModel = require('../model/ResourcesModel');

var exportModelAttributesFactory = require('../factory/exportModelAttributesFactory');

var exportTypesEnum = require('../enum/exportTypesEnum');

var i18nMessageUtil = require("runtime_dependencies/js-sdk/src/common/util/i18nMessage");

var secureKeyTypeEnum = require('../../import/enum/secureKeyTypeEnum');
...
var MAX_LENGTH = 256,
  ...
  toExportTask: function toExportTask() {
    ...
    !this.get("includeDependencies") && repositoryExportOptions.parameters.push("skip-dependent-resources"); // drop parent org prefix from paths, add required attributes if exporting from sub-org and includeFullResourcePath is unchecked
    ...
    return this.type !== exportTypesEnum.REPOSITORY ? tenantExportOptions : repositoryExportOptions;
  },
  ExportOptions: function ExportOptions() {
    if (this.get("keyType") === secureKeyTypeEnum.CUSTOMKEY) {
      return {
        'keyAlias': this.get("keyAlias")
      };
    } else if (this.get("keyType") === secureKeyTypeEnum.UNIVERSALKEY) {
      return {
        'keyAlias': this.get("keyType")
      };
    } else {
      return {
        'keyAlias': secureKeyTypeEnum.DEFAULTKEY
      };
    }
  },
  reset: function reset(opts, type) {
    this.type = type;
...
function getResourceTypes(exportModel) {
  var res = [],
      dfd = new $.Deferred();
  !exportModel.get("everything") && someResourceIsChecked(exportModel) ? exportModel.resources.fetch().done(function (resources) {
