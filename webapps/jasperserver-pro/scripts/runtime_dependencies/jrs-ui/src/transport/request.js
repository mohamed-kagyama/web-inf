...
module.exports = function () {
  ...
  var origin = getOriginFromUrl(window.location.href); // TODO: observableRequest functionality is temporary here. This should be revised after Amber release.
  ...
  return $.ajax(mergedOptions).fail(isSameOrigin(serverOrigin) && checkSessionExpiration).fail(function (jqXHR, textStatus, errorThrown) {
  ...
  }).fail(_.partial(triggerEvent, "request:failure")).done(_.partial(triggerEvent, "request:success")).done(arguments[1]);
};
