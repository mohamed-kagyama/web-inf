...
var TemplateEngine = require('../../components/components.templateengine');

var groupsEnum = require('../../serverSettingsCommon/enum/serverSettingGroupsEnum');

var levelIdEnum = require('../../attributes/enum/levelIdEnum');
...
var AttributesCollection = Backbone.Collection.extend({
  ...
  setContext: function setContext(context, forceRefresh) {
    context = context || {
  ...
  validateSearch: function validateSearch(model, newModels, omitId, groupsSearch) {
  ...
  filterInheritedAttributes: function filterInheritedAttributes(data) {
  ...
  search: function search(models, newModels, recursive, omitId, groups) {
    recursive = recursive || '';
    groups = groups || '';
  ...
  addItemsToCollection: function addItemsToCollection(attributes) {
  ...
  addNew: function addNew(attribute) {
    attribute = attribute || {};
    var model = new this.model(attribute);
    this.add(model);
    return model;
  },
  _modelsToJSON: function _modelsToJSON(models, omitValue) {
  ...
  _successSearchCallback: function _successSearchCallback(models, newModels, data) {
  ...
  _concatGroups: function _concatGroups() {
    var groups = '';
  ...
  _concatNames: function _concatNames(models, omitId) {
    models = _.isArray(models) ? models : [models];

    var nameStr = '',
        self = this,
        getNameParameter = function getNameParameter(name) {
