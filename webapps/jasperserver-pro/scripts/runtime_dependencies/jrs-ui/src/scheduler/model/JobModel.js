...
var globalConfig = require("settings!globalConfiguration");
...
var getTZOffset = _utilUtilsCommon.getTZOffset;

function _typeof(obj) { if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") { _typeof = function _typeof(obj) { return typeof obj; }; } else { _typeof = function _typeof(obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }; } return _typeof(obj); }

var EMAIL_SEPARATOR = ",";
var SERVICE_DATE_PATTERN_MOMENTJS_FORMAT = "YYYY-MM-DD HH:mm";
var ftpIsProcessings = false; // This allows us to simply check the failed folder URI on sequent validation calls and avoid the extra requests to server.
...
var convertFormatFromDateTimePickerToMomentJs = function convertFormatFromDateTimePickerToMomentJs(format) {
  var i,
      conversion = [{
    id: 0,
    value: "dd",
  ...
  for (i = 0; i < conversion.length; i++) {
    format = format.replace(new RegExp(conversion[i].value, "g"), "{" + conversion[i].id + "}");
  }

  for (i = 0; i < conversion.length; i++) {
    format = format.replace(new RegExp("\\{" + conversion[i].id + "\\}", "g"), conversion[i].result);
  }

  return format;
};

var momentDateFormat = convertFormatFromDateTimePickerToMomentJs(config.calendar.timepicker.dateFormat); // time format is the same for momentjs which we use for DateTimePicker
// 'calendar.timepicker.timeFormat' is referencing 'calendar.time.format' from jasperserver_config.properties file

var momentTimeFormat = config.calendar.timepicker.timeFormat.replace(":ss", ""); // in Scheduler we don't use "seconds", so momentjs should not take seconds into account
...
module.exports = Backbone.Model.extend({
  ...
  validateAll: function validateAll(editMode) {
    var self = this,
        dfr = $.Deferred(); // 1) run server requests validators first

    this.runServerValidationRequests().always(function (result) {
      // 2) run standard validation
      var isValid = self.isValid(editMode); // trigger error event after self.isValid() because standard model validation clears all errors.

      if (result && result.errorCode) {
        isValid = false;
        self.trigger('invalid', [result], {
          switchToErrors: false
        });
      }
  ...
  isValid: function isValid(editMode) {
  ...
  update: function update(key, value) {
    if ('object' === _typeof(value)) {
      ...
      for (var i in value) {
        if (value.hasOwnProperty(i)) if (value[i] === undefined) delete value[i];
      }
    }
  ...
  sync: function sync(method, model, options) {
    ...
    if (method === 'update') method = 'create';else if (method === 'create') method = 'update';
    options || (options = {});
  ...
  parse: function parse(data) {
    if (data.trigger) {
      ...
      if ('simpleTrigger' in data.trigger) data.trigger.type = 'simpleTrigger';else if ('calendarTrigger' in data.trigger) data.trigger.type = 'calendarTrigger'; // merge trigger sub-values
      ...
      if (data.trigger.type === 'simpleTrigger' && data.trigger.recurrenceInterval === null) data.trigger.type = 'none';else if (data.trigger.type === 'simpleTrigger') data.trigger.type = 'simple';else if (data.trigger.type === 'calendarTrigger') data.trigger.type = 'calendar';else data.trigger.type = 'none'; // adjust some fields "by default" if the current recurrence type is none
    ...
    if (typeof data.repositoryDestination === "undefined" || data.repositoryDestination === null) {
      ...
      if (!data.repositoryDestination.timestampPattern) data.repositoryDestination.timestampPattern = "yyyyMMddHHmm";
    }
    ...
    if (typeof data.mailNotification.resultSendType === "undefined" || data.mailNotification.resultSendType === null) {
      ...
      if (rs !== "SEND" && rs !== "SEND_ATTACHMENT" && rs !== "SEND_ATTACHMENT_NOZIP" && rs !== "SEND_EMBED" && rs !== "SEND_ATTACHMENT_ZIP_ALL" && rs !== "SEND_EMBED_ZIP_ALL_OTHERS") {
        data.mailNotification.resultSendType = defaultResultSendTypeValue;
      }
    } // convert arrays into strings


    var keys = ["toAddresses", "ccAddresses", "bccAddresses"],
        i,
        from,
        to;
  ...
  toJSON: function toJSON() {
    ...
    if (data.trigger) {
      ...
      if (data.trigger.type === 'none') {
        ...
        data.trigger.recurrenceIntervalUnit = null; // remove fields from 'Calendar' section
        ...
        data.trigger.type = 'simpleTrigger'; // occurrence count selected
        ...
        data.trigger.type = 'calendarTrigger'; //months block
        ...
        data.trigger.monthDays = data.trigger.monthDays.replace(/ /g, ""); // remove this field if it's not specified
    ...
    if (data.repositoryDestination) {
      if (data.repositoryDestination.outputFTPInfo) {
        ...
        if ('enabled' in data.repositoryDestination.outputFTPInfo) delete data.repositoryDestination.outputFTPInfo.enabled;
        if ('sshKeyEnabled' in data.repositoryDestination.outputFTPInfo) delete data.repositoryDestination.outputFTPInfo.sshKeyEnabled;
      }
  ...
  state: function state(_state) {
    ...
    this.get('state').value = _state ? jobStateEnum.NORMAL : jobStateEnum.PAUSED;
    this.trigger('change'); // call backbone sync method manually

    return Backbone.sync.call(this, 'update', data, {
      url: config.contextPath + '/rest_v2/jobs/' + (_state ? 'resume' : 'pause'),
      type: 'POST'
    });
  },
  // get parameters for job
  loadParameters: function loadParameters(reportUnitURI) {
    reportUnitURI = reportUnitURI || this.get('source').reportUnitURI;
    var self = this,
        url = config.contextPath + '/rest_v2/reports' + encodeURI(reportUnitURI) + '/inputControls/'; // call backbone sync method manually
  ...
  checkSaveValidation: function checkSaveValidation(options) {
    options = options || {};
    var dfr = $.Deferred();
    var testsPassed = 0;

    var onTestPassing = function onTestPassing() {
      testsPassed++;
    ...
    var onTestFailure = function onTestFailure() {
      dfr.reject();
    };
  ...
  testDates: function testDates(editMode) {
    var results = [],
        dfr = $.Deferred();
  ...
  testFTPConnection: function testFTPConnection(callback) {
    if (ftpIsProcessings) {
    ...
    ftpIsProcessings = true; // disable FTP button

    $("#ftpTestButton").addClass("checking"); // clear the erro while we are performing checking...

    $("[data-field=ftpTest]").text(""); // call backbone sync method manually
    ...
    return Backbone.sync.call(this, 'update', data, {
      ...
      success: function success(data, xhr) {
        ftpIsProcessings = false; // enable FTP button
  ...
  validateParametersIC: function validateParametersIC() {
    var self = this,
        dfr = $.Deferred(),
  ...
  runServerValidationRequests: function runServerValidationRequests() {
    var dfr = $.Deferred();
    var testsPassed = 0;
    ...
    var onTestFailure = function onTestFailure(result) {
      dfr.reject(result);
    };
  ...
  testSshKey: function testSshKey() {
    var dfr = $.Deferred();
    ...
    var sshKey = ftpInfo.sshKey; // skip server requests if sshKey value is empty or incorrect
  ...
  testOutputRepositoryFolder: function testOutputRepositoryFolder() {
    var dfr = $.Deferred();
    ...
    var folder = this.get('repositoryDestination').folderURI; // check for empty
  ...
  testOutputHostFileSystemFolder: function testOutputHostFileSystemFolder() {
    var self = this;
    var dfr = $.Deferred();
  ...
  checkPermissionOnFolder: function checkPermissionOnFolder(folder, callback) {
  ...
  //
  resource: function resource(type, uri, callback) {
    // call backbone sync method manually
    return Backbone.sync.call(this, 'read', new Backbone.Model(), {
      url: config.contextPath + '/rest_v2/resources' + uri,
      headers: {
        'Accept': 'application/repository.' + type + '+json'
      },
      type: 'GET',
      success: function success(data, xhr) {
        if ('function' === typeof callback) callback(undefined, data);
      },
      error: function error(err) {
        if ('function' === typeof callback) callback(err);
      }
  ...
  parseUri: function parseUri(uri) {
    // private variables
    var match; // check first char of uri, make it / in any case

    if (uri[0] !== '/') uri = '/' + uri; // check for pattern and return parsed values if matches
  ...
  createFromUri: function createFromUri(uri) {
    // clear all errors on the form
    this.trigger('clearAllErrors'); // clear model

    this.clear({
      silent: true
    }); // parse uri components

    uri = this.parseUri(uri); // load defaults from jrsConfigs.jsp

    var jrsConfigDefaults = _.extend({}, config.reportJobEditorDefaults); // set new model attributes
  ...
  parseIntervals: function parseIntervals(str, type) {
    str = str || false;
    if (!str) return false;
    str = str.replace(/ /g, "");
    var strs = str.split(","),
    ...
    for (i = 0; i < strs.length; i++) {
      ...
      tmp = strs[i].split("-");
      if (!this.isNumeric(tmp[0], {
        allowZero: true
      })) return false;
      if (!this.isNumeric(tmp[1], {
        allowZero: true
      })) return false;
  ...
  ifPastDate: function ifPastDate(date, date_tz) {
  ...
  isHostName: function isHostName(hostname) {
  ...
  validateEmails: function validateEmails(emails) {
    ...
    for (var i = 0; i < emails.length; i++) {
      if (!this.isEmail(emails[i])) return false;
    }

    return true;
  },
  isNumeric: function isNumeric(val, cfg) {
    var i;
    if (!val) return false;
    cfg = cfg || {};
