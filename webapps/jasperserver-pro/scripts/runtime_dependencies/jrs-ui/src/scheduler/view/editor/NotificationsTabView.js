...
var notificationsTabTemplate = require("text!../../template/editor/notificationsTabTemplate.htm");
...
module.exports = Backbone.View.extend({
    ...
    this.model.on('change:outputFormats', function (model, value) {
      ...
      if (!contains) {
        ...
        if (c && c.resultSendType && (c.resultSendType === 'SEND_EMBED' || c.resultSendType === 'SEND_EMBED_ZIP_ALL_OTHERS')) {
          ...
          model.update('mailNotification', {
            resultSendType: changeTo
          });
          return;
        }
      }
  ...
  includeHtmlReport_DisabledEnabled: function includeHtmlReport_DisabledEnabled(model) {
    var enabled = true;
    var mn = model.get('mailNotification');
  ...
  message_suc_DisabledEnabled: function message_suc_DisabledEnabled(model) {
    var enabled = true;
    var mn = model.get('mailNotification');
  ...
  asRepoLinks_DisabledEnabled: function asRepoLinks_DisabledEnabled(model) {
    var enabled = true;
    var rd = model.get('repositoryDestination');
  ...
  to_suc_Change: function to_suc_Change(evt) {
    this.model.update('mailNotification', {
      toAddresses: {
        address: $(evt.target).val()
      }
    });
  },
  cc_suc_Change: function cc_suc_Change(evt) {
    this.model.update('mailNotification', {
      ccAddresses: {
        address: $(evt.target).val()
      }
  ...
  bcc_suc_Change: function bcc_suc_Change(evt) {
    this.model.update('mailNotification', {
      bccAddresses: {
        address: $(evt.target).val()
      }
  ...
  subject_suc_Change: function subject_suc_Change(evt) {
    this.model.update('mailNotification', {
      subject: $(evt.target).val()
    });
  },
  message_suc_Change: function message_suc_Change(evt) {
    this.model.update('mailNotification', {
      messageText: $(evt.target).val()
    });
  },
  setResultSendType: function setResultSendType(evt) {
    ...
    this.model.update('mailNotification', {
      resultSendType: newVal
    });
  },
  dont_send_empty_report_Change: function dont_send_empty_report_Change(evt) {
    this.model.update('mailNotification', {
      skipEmptyReports: $(evt.target).is(':checked')
    });
  },
  job_status_to_Change: function job_status_to_Change(evt) {
    this.model.update('alert', {
      toAddresses: {
        address: $(evt.target).val()
      }
    });
  },
  job_status_subject_Change: function job_status_subject_Change(evt) {
    this.model.update('alert', {
      subject: $(evt.target).val()
    });
  },
  twoCheckboxes_Change: function twoCheckboxes_Change(evt) {
    var v1 = this.$el.find('[name=send_success_notification]').is(':checked'),
  ...
  job_status_success_message_Change: function job_status_success_message_Change(evt) {
    this.model.update('alert', {
      messageText: $(evt.target).val()
    });
  },
  job_status_failed_message_Change: function job_status_failed_message_Change(evt) {
    this.model.update('alert', {
      messageTextWhenJobFails: $(evt.target).val()
    });
  },
  include_report_Change: function include_report_Change(evt) {
    this.model.update('alert', {
      includingReportJobInfo: $(evt.target).is(':checked')
    });
  },
  include_stack_trace_Change: function include_stack_trace_Change(evt) {
    this.model.update('alert', {
      includingStackTrace: $(evt.target).is(':checked')
    });
  }
});
