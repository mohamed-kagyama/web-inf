...
module.exports = Backbone.View.extend({
  ...
  initialize: function initialize(options) {
    ...
    this.jobsViewCollection = this._getJobsCollection(this.options); // handle collection changes
  ...
  render: function render() {
    var searchTerm = this.jobsViewCollection.getSearchingTerm();
  ...
  renderCollection: function renderCollection() {
    ...
    if (this.jobsViewCollection.length) {
      ...
      jobsContainer.append($(_.template(nothingToDisplayTemplate, {
        message: this.options.masterViewMode ? i18n['report.scheduling.list.no.jobs'] : nothingToDisplayMessage
      })));
    }
  },
