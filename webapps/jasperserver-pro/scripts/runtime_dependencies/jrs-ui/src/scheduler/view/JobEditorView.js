...
module.exports = Backbone.View.extend({
  ...
  tabChangeClickEvent: function tabChangeClickEvent(event) {
    ...
    this.model.validateAll(this.editMode).always(function () {
      ...
      var errors = _this.$el.find('[name=' + _this.currentActiveTabName + ']').find('.error'); // don't switch current tab if one has error(s)
      ...
      _this.changeActiveTab(tabName); // clear errors on the tab if any


      _this.clearAllValidationErrors();
    });
  },
    ...
    if (this.model._fetched) {
      ...
      this.model.fetch({
        success: function success() {
          // set proper title
          _this2.setTitle(_this2.model.get('label'));
        }
      });
    }
  },
  ...
  sourceKeyInModelChanged: function sourceKeyInModelChanged(model, value) {
    ...
    this.$el.find('.pageHeader-subtitle-path').find('.pageHeader-subtitle-path-text').text(value.reportUnitURI); //this.$el.find('.schedule_for').find('.path').text(value.reportUnitURI);
  },
  // ==================================================================================================
  // Save methods...
  ...
  startSavingProcess: function startSavingProcess() {
    ...
    this.model.validateAll(this.editMode).done(function () {
      _this4._prepareSaveDialog();

      _this4.saveDialog.startSaveDialog();
    }).fail(function () {
      _this4.changeActiveTab('error');
    });
  },
  ...
  _onSaveFail: function _onSaveFail(model, xhr, options) {
    ...
    _.each(response, function (error) {
      ...
      if (error.errorCode === 'resource.not.found') {
        ...
        this.listenTo(alertDialog, 'close', function () {
          _this5.trigger('errorEditingJob');
        });
        alertDialog.open();
        justExit = true;
        return;
  ...
  validModelListener: function validModelListener(messages) {
    ...
    _.each(messages, function (message) {
      _this6.showValidationMessage('success', message);
    });
  },
  invalidModelListener: function invalidModelListener(errors, options) {
    ...
    _.each(errors, function (error) {
      _this7.showValidationMessage('error', error);
    });
