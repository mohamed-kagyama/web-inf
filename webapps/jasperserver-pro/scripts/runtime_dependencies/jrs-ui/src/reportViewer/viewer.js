...
require("csslink!./jasperreports/jive/theme/jive.csslink.css");
...
var Viewer = function Viewer(options) {
  ...
  $('body').on({
    'jive.initialized': function jiveInitialized(evt, jive) {
      it.jive = jive;
    },
    'jive.inactive': function jiveInactive() {
  ...
  if ($('script#reportSearchText').length > 0) {
    ...
    $('button#search_report_button').on('click', function (evt) {
      $('input#search_report').trigger($.Event("keydown", {
        which: 13
      }));
    });
    ...
    btnSearchNext.on('click', function (evt) {
      var spans = $('.jr_search_result'),
          ...
          i,
          ln,
          elem; // search results are sorted by page number in ascending order
  ...
  $('button#bookmarksDialog').on('click', function () {
    it.bookmarksContainer && it.bookmarksContainer.toggle().offset({
      left: 10,
      top: 95
    });
  });
...
Viewer.privateMethods = {
  // bookmarks
  prepareBookmarks: function prepareBookmarks(bookmarks) {
    var it = this,
        container = $('div#jr_bookmarks'),
        body,
        html; // if bookmarks already in place, do nothing
    ...
    if (!container.length) {
      container = $("<div id='jr_bookmarks' class='panel dialog overlay moveable sizeable' style='width: 250px; height: 600px; z-index: 2000; display: none'>" + "<div class='sizer diagonal ui-resizable-handle ui-resizable-se'></div>" + "<div class='content hasFooter'>" + "<div class='header mover'>" + "<div class='closeIcon'></div>" + "<div class='title'>" + i18n['net.sf.jasperreports.components.headertoolbar.bookmarks.dialog.title'] + "</div>" + "</div>" + "<div class='body' style='top: 29px; bottom: 24px; padding: 0'></div>" + "<div class='footer' style='height: 24px'></div>" + "</div>" + "</div>").appendTo('body');
  ...
  exportBookmark: function exportBookmark(bookmark) {
    var it = this,
        html = "",
  ...
  cleanupBookmarks: function cleanupBookmarks() {
    this.bookmarksContainer && this.bookmarksContainer.find('.body').empty();
  },
  // report parts
  prepareReportParts: function prepareReportParts(parts, forceRedraw) {
    var it = this,
        partsContainer = $('ul#reportPartsContainer'),
        html = ""; // if parts already in place, do nothing
  ...
  exportPart: function exportPart(part) {
    var it = this,
        name = part.name;
    ...
    return "<li class='leaf'>" + "<div class='button reportPart' role='button' js-navtype='button' data-pageindex='" + part.idx + "' data-title='true' aria-label='" + name + "' tabindex='-1'><span>" + name + "</span></div></li>";
  },
  getNextPart: function getNextPart(tab) {
    var it = this,
        parts = it.getReportParts(),
        indexOfTab = it.partsStartIndex.indexOf(tab.data('pageindex'));
    return parts[indexOfTab + 1];
  },
  getPreviousPart: function getPreviousPart(tab) {
    var it = this,
        parts = it.getReportParts(),
        indexOfTab = it.partsStartIndex.indexOf(tab.data('pageindex'));
    return parts[indexOfTab - 1];
  ...
  getReportParts: function getReportParts() {
    ...
    if (it.reportInstance.reportComponents && it.reportInstance.reportComponents.reportparts && it.reportInstance.reportComponents.reportparts[0].config.parts.length > parts.length) {
      parts = it.reportInstance.reportComponents.reportparts[0].config.parts;
    }

    return parts;
  },
  markActiveReportPart: function markActiveReportPart() {
    var it = this,
        activePartIndex = -1,
  ...
  restoreLastActiveSearchHighlight: function restoreLastActiveSearchHighlight() {
    var it = this,
        spans;
  ...
  getOptionsMenu: function getOptionsMenu(feature) {
    var it = this,
    ...
    optionsContainer = menu.find('ul'); // cleanup
  ...
  // zoom
  getZoomByValue: function getZoomByValue(val) {
    var zf = this.features.zoom,
        reportViewFrame = $('div#reportViewFrame div.body:first'),
        jrPage = $('table.jrPage'),
        zum; // fix case when report is viewed with viewAsDashboardFrame=true
  ...
  processZoomOption: function processZoomOption(val) {
    var it = this,
        zum,
        literalValue,
  ...
  propagateZoom: function propagateZoom(o) {
    ...
    if (this.reportInstance && this.reportInstance.components) {
      ...
      $.each(components, function (i, componentArray) {
        if (componentArray.length > 0) {
          uimoduleName = componentArray[0].config.uimodule; // propagate zoom only to the loaded ui modules
  ...
  zoomIn: function zoomIn(amount, literalValue) {
    var it = this,
    ...
    if (actualTableWidth < containerWidth && tableWidth * newZoomLevel > containerWidth) {
      // new scaling will overflow the container
    ...
    it.features.zoom.levels.current.value = newZoomLevel; // this is used by jive to "know" about the actual size of the report

    it.reportInstance.zoom = zoom;
    $('#zoom_value').val((newZoomLevel * 100).toFixed(0) + '%');
    it.propagateZoom(zoom);
  },
  zoomOut: function zoomOut(amount, literalValue) {
    var it = this,
        currentZoom = it.features.zoom.levels.current.value,
        amount = amount || 0.1,
        table = $('table.jrPage'),
  ...
  zoomTo: function zoomTo(zoomLevel, literalValue) {
    var it = this,
        currentZoom = it.features.zoom.levels.current.value,
        newZoomLevel;
    ...
    if (zoomLevel > currentZoom) {
      ...
      it.features.zoom.levels.current.literal = literalValue || null; // necessary for components that rely on zoom level
  ...
  saveCurrentLevel: function saveCurrentLevel() {
  ...
  scrollElementIntoView: function scrollElementIntoView(elem) {
    this.reportInstance.eventManager.triggerEvent("beforeSearchAdvance");
    elem.scrollIntoView(false);
  },
  render: function render(htmlOutput) {
    var it = this; // Due to the html markup of the AdHoc Chart, there is an issue when the
    ...
    $('table.jrPage').prop('tabindex', '8'); // Prevent screen-readers from guessing that our table is used only for layout purposes,
    ...
    try {
      ...
      $.each(scripts, function (i, v) {
        ...
        if (it.reportInstance.loader.jasperPrintName !== window.Report.jasperPrintName) {
          it.reportInstance.loader.jasperPrintName = window.Report.jasperPrintName;
        }
      });
    } catch (ex) {
      alert(ex);
  ...
  setupEventsForReport: function setupEventsForReport(report) {
    ...
    report.on("reportHtmlReady", function () {
      ...
      if (it.jive) {
        if (!it.jive.active || it.isUndoRedo) {
          ...
          /* FIXME: remove or place inside CSS file */

          it.dfds['jive.inactive'].resolve(); // Give the table a tabindex so that Standard Navigation can work with it.
    ...
    report.on("beforeAction", function (evt) {
      ...
      scrollContainer.scrollTop(0); // search is an action, but the search controls should not be reset before it
    ...
    report.on("error", function (evt) {
      if (evt.data && evt.data.error && evt.data.message) {
        ...
        console && typeof console.log === 'function' && console.log(evt.data.error);
    ...
    report.on("search", function (evt) {
      ...
      if (data && data.result.actionResult.searchResults && data.result.actionResult.searchResults.length) {
        ...
        it.search.results = results; // if we have results for the current page, just refresh it
    ...
    report.on("reportFinished", function () {
      if (this.status.pageFinal) {
        ...
        window.Report.snapshotSaveStatus = this.status.originalStatus.snapshotSaveStatus;
        window.Report.lastPageIndex = this.status.originalStatus.lastPageIndex;
        window.Report.reportRefreshed();
      } else {
        this.gotoPage(this.currentpage);
      }
    });
    report.on('componentsLoaded', function () {
      /*
      ...
      function calculateHighchartsParentContainerHeight() {
        if (window.location.href.indexOf("viewAsDashboardFrame=true") > -1) {
          try {
            ...
            return $(document).height() ? $(document).height() - hcParentContainer.position().top : 400;
          }
        }
      ...
      it.isAdhocView = false;
      $.each(this.loadedComponents, function (key, compMeta) {
        if (compMeta.type === 'chart' && compMeta.hcinstancedata && compMeta.hcinstancedata.services && compMeta.hcinstancedata.services.length && compMeta.hcinstancedata.services[0].service === 'adhocHighchartsSettingService') {
          it.isAdhocView = true;
          it.recreateChartOptionsOnResize = compMeta.hcinstancedata.services[0].data && compMeta.hcinstancedata.services[0].data.recreateOptionsOnResize;
        }
      });
      ...
      if (it.isAdhocView) {
        ...
        allRows.each(function (i, row) {
          // each row below the one with the chart must have 0px in height
        ...
        if (!it.chartResizeListener) {
          it.chartResizeListener = true;
          /*
              Setup resize events
          */
          ...
          $(window).on('customResizeEnd', function () {
            ...
            $.each(it.reportInstance.components.chart, function () {
              this.render({
                recreateConfig: it.recreateChartOptionsOnResize,
                ignoreSize: true
              });
            });
          });
    ...
    report.on('componentsRegistered', function () {
      ...
      var jrPage = $('table.jrPage'),
          hcParentContainer = jrPage.find('div.highcharts_parent_container'); // resolve deferred zoom (necessary for reports with HC charts to be done after the Adhoc ones do their CSS mods for resizing)
      ...
      if (components.chart) {
        hcParentContainer.attr("js-stdnav", "false");
        $.each(components.chart, function () {
          var el = $('#' + this.config.hcinstancedata.renderto).length,
              self = this; // delay Hicharts rendering until container will be fully initialized with sizes
          ...
          if (it.isAdhocView) {
            el && it.container.height() && setTimeout(function () {
              self.render();
            }, 1);
          } else {
      ...
      if (components.hyperlinks && components.hyperlinks.length) {
        ...
        $.each(hyperlinksByType, function (key, links) {
          require(["jr." + key], function (HyperLinkHandler) {
            ...
            if (failedId && console && typeof console.error === 'function') {
              // TODO: replace with logger
      ...
      window.Report.reportRefreshed(); // very dirty hack to fix mystical issue http://bugzilla.jaspersoft.com/show_bug.cgi?id=39694
  ...
  handleReportBookmarks: function handleReportBookmarks(componentsContainer) {
    var it = this;
  ...
  handleReportParts: function handleReportParts(componentsContainer) {
    var it = this;
    ...
    if (componentsContainer.reportparts && componentsContainer.reportparts.length) {
      var parts = componentsContainer.reportparts[0].config.parts,
          forceRedraw = true; // check if we have a different number of parts than existing
  ...
  encodeString: function encodeString(stringToEncode) {
    return $("<div></div>").text(stringToEncode).html();
  }
};
Viewer.publicMethods = {
  loadReport: function loadReport() {
    ...
    return it.reportInstance.init()["catch"](function () {
      var _$$Deferred;

      _this.loading = false;
      return (_$$Deferred = $.Deferred()).reject.apply(_$$Deferred, arguments);
    });
  },
  exit: function exit() {
