...
jQuery.extend(Report, {
  ...
  reportRefreshed: function reportRefreshed(silentUpdate) {
    ...
    */
    ...
    var emptyMessage = jQuery(jQuery("#emptyReportID p.message")[1]).html();
    var emptyMessageLength = emptyMessage && emptyMessage.trim().length;
    ...
    if (emptyReportState && emptyReportState.length > 0 && emptyMessageLength === 0) {
...
jQuery.extend(Report, {
  exportReport: function exportReport(type, formAction) {
    ...
    while (exportForm.hasChildNodes()) {
    ...
    var selection = Controls.viewModel.get('selection');
