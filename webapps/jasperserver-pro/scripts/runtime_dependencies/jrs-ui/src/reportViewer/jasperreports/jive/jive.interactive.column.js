...
require("runtime_dependencies/js-sdk/src/common/extension/jQueryTimepickerExtension");
...
jive.interactive.column = {
  ...
  actions: {
    format: {
      icon: 'formatIcon',
      title: jive.i18n["net.sf.jasperreports.components.headertoolbar.title.format"],
      actions: {
        formatHeaderAndValues: {
          label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.formatting"],
          fn: 'formatHeader'
        },
        hideColumn: {
          label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.hidecolumn"],
          fn: 'hide',
          arg: '{"hide":true}'
        },
        showColumns: {
          label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.showcolumns"],
          actions: {
            showAll: {
              label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.showcolumns.all"],
              fn: 'hide',
              arg: '{"hide":false,"column":"all"}'
            }
          }
        }
      }
    ...
    filter: {
      icon: 'filterIcon',
      title: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.columnfilters"],
      fn: 'filter'
    },
    sortAscending: {
      icon: 'sortAscIcon',
      title: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.sortasc"],
      fn: 'sort',
      arg: ['Asc']
    },
    sortDescending: {
      icon: 'sortDescIcon',
      title: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.sortdesc"],
      fn: 'sort',
      arg: ['Desc']
    }
  },
  ...
  initColumns: function initColumns(Table) {
    var it = this,
        c,
        i,
        lt;
    ...
    it.basicFormatForm.elements[1][0][0].groups = [{
      name: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.extfonts"],
      values: []
    }];
    $.each(it.fonts.extension, function (i, v) {
      it.basicFormatForm.elements[1][0][0].groups[0].values.push([v, v]);
    });
    it.basicFormatForm.elements[1][0][0].groups.push({
      name: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.sysfonts"],
      values: []
    });
    ...
    this.count[tableUuid] = $.grep(Table.columns, function (c) {
      return c;
    }).length;
    ...
    for (i in allColumns) {
      ...
      if (c.interactive) {
        menu.actions[c.uuid] = {
          label: jive.decodeHTML(c.label),
          fn: 'hide',
          arg: '{"hide":false, "columnUuid": "' + c.uuid + '"}'
        };
      }
  ...
  prepareVisibleColumnsMoveData: function prepareVisibleColumnsMoveData(tableUuid) {
    var jqReportContainer = this.reportInstance.config.container;
    ...
    for (i = 0; i < tableCols.length; i++) {
      var c = tableCols[i];
      var absLeft = c.jo.offset().left + this.scrollContainer.scrollLeft(),
  ...
  getJoForCurrentSelection: function getJoForCurrentSelection(jo) {
    var result = null;
  ...
  getColumnByUuid: function getColumnByUuid(columnUuid, tableUuid) {
  ...
  getInteractiveElementFromProxy: function getInteractiveElementFromProxy(cellJo) {
    ...
    cellJo.parents().each(function (i, v) {
      ...
      if (headerJo && headerJo.length > 0 && headerJo.offset().top < cellJoOffsetTop) {
        // the header must be above the cell
        return false; // break each
      }
  ...
  zoom: function zoom(o) {
    var it = this; // need to recalculate all visible columns dimensions after each zoom
  ...
  onDrag: function onDrag(evt, ui) {
    var pageX = evt.pageX + this.scrollContainer.scrollLeft(),
        ...
        isLeftToRight,
        hoverCol,
        newColIndex = null; // determine move direction relative to selected column
    ...
    for (i = 0; i < ln; i++) {
      ...
      if (pageX <= colMoveData.right) {
        ...
        if (pageX <= refColMiddle) {
          // move left, relative to column middle
          ...
          // move right, relative to column middle
    ...
    this.colToMoveToIndex = newColIndex; // Try to auto scroll the container when trying to move at edges
  ...
  resize: function resize(width) {
    ...
    jive.hide();
    jive.selected.ie.resize({
      width: w
    });
  },
  sort: function sort(argv) {
    jive.hide();
    jive.selected.ie.sort({
      order: argv[0]
    });
  },
  filter: function filter() {
    ...
    jive.ui.dialog.show(jive.i18n["net.sf.jasperreports.components.headertoolbar.title.filtercolumn"] + ': ' + label, ['columnfilter']);
  },
  formatHeader: function formatHeader() {
    var label = jive.selected.ie.config.columnLabel;
    !label.length && (label = "#" + (jive.selected.ie.config.columnIndex + 1));
    jive.ui.dialog.show(jive.i18n["net.sf.jasperreports.components.headertoolbar.title.formatcolumn"] + ': ' + label, ['basicFormat', 'columnConditionalFormatting']);
  },
  ...
  setGenericProperties: function setGenericProperties(obj) {
  ...
  showCurrencyDropDown: function showCurrencyDropDown() {
    jive.selected.form.inputs['currencyBtn1'].showOptions();
  },
  applyCurrencyFormat: function applyCurrencyFormat(args) {
    var it = this,
  ...
  togglePercentageFormat: function togglePercentageFormat() {
    var it = this;
    $('#formatPattern').children().each(function (i, optElem) {
      var opt = $(optElem);
  ...
  toggleCommaFormat: function toggleCommaFormat() {
    var it = this;
    $('#formatPattern').children().each(function (i, optElem) {
      var opt = $(optElem);
  ...
  remDecimal: function remDecimal() {
    var it = this;
    $('#formatPattern').children().each(function (i, optElem) {
      var opt = $(optElem);
  ...
  numberFormat: {
     ...
     */
    applyNegativeNumberPattern: function applyNegativeNumberPattern(negPattern) {
    ...
    addRemovePercentageForNumber: function addRemovePercentageForNumber(numberExp, booleanAdd) {
    ...
    addRemoveCurrencySymbol: function addRemoveCurrencySymbol(exp, booleanAdd, currencySymbol) {
...
jive.interactive.column.columnFilterForm = {
  ...
  elements: [[[{
    type: 'radio',
    id: 'clearFilter',
    label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.clearfilter.true"],
    value: 'true'
  }]], [[{
    type: 'radio',
    id: 'clearFilter',
    label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.clearfilter.false"],
    value: 'false',
    colspan: 4
  }], [{
    type: 'list',
    id: 'filterTypeOperator',
    values: []
  }, {
    type: 'text',
    id: "fieldValueStart",
  ...
  onCreate: function onCreate(jo) {
    ...
    it.jc.filterType = $('#filterTypeOperator').change(function () {
      ...
      if (currentVal.indexOf('BETWEEN') >= 0) {
      ...
      if (currentVal.indexOf('NULL') >= 0) {
        it.jc.filterStart.parent().parent().hide();
        it.jc.filterStart.prop('disabled', true);
      } else {
        it.jc.filterStart.parent().parent().show();
    ...
    $('input[name="clearFilter"]').change(function () {
      ...
      for (var p in it.jc) {
        it.jc.hasOwnProperty(p) && $(this).val() == 'true' ? it.jc[p].prop('disabled', true) : it.jc[p].prop('disabled', false);
      }
  ...
  onShow: function onShow() {
    ...
    $.each(options, function (i, option) {
      it.jc.filterType.append($("<option></option>", {
        value: option.key,
        text: option.val
      }));
    });
    ...
    for (p in it.jc) {
      it.jc.hasOwnProperty(p) && filterOff ? it.jc[p].prop('disabled', true) : it.jc[p].prop('disabled', false);
    }
    ...
    if (filtertype !== 'boolean') {
      if (metadata.filterTypeOperator && metadata.filterTypeOperator.indexOf('NULL') >= 0) {
        it.jc.filterStart.prop('disabled', true);
        it.jc.filterStart.closest('td').hide();
      } else {
        it.jc.filterStart.closest('td').show();
...
jive.interactive.column.basicFormatForm = {
  actionDataCache: {},
  title: jive.i18n["net.sf.jasperreports.components.headertoolbar.title.basicformat"],
  name: 'basicFormat',
  method: 'get',
  prevApplyTo: null,
  elements: [[[{
    type: 'text',
    id: 'headingName',
    label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.headingtext"],
    value: '',
    colspan: 4
  }]], // common format
  [[{
    type: 'list',
    id: 'fontName',
    label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.fontname"],
    values: [],
    freeText: true,
    size: 6,
    rowspan: 2
    ...
    type: 'list',
    id: 'fontSize',
    label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.fontsize"],
    values: [],
    freeText: true,
    size: 6,
    rowspan: 2
    ...
    type: 'buttons',
    label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.style"],
    items: [{
      type: 'checkbox',
      id: 'fontBold',
      value: 'bold',
    ...
    type: 'buttons',
    label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.color"],
    items: [{
      type: 'backcolor',
      id: 'fontBackColor',
      bIcon: 'backgroundColorIcon',
      title: jive.i18n["net.sf.jasperreports.components.headertoolbar.title.fontbackcolor"],
      drop: true,
      ...
      id: 'fontColor',
      bIcon: 'fontColorIcon',
      title: jive.i18n["net.sf.jasperreports.components.headertoolbar.title.fontcolor"],
      drop: true
    }]
  }], [{
    type: 'buttons',
    label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.alignment"],
    items: [{
      type: 'radio',
      id: 'fontAlign',
      value: 'Left',
  ...
  [[{
    type: 'list',
    id: 'formatPattern',
    label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.formatpattern"],
    freeText: true,
    values: [],
    colspan: 2,
    size: 4,
    ...
    label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.numberformat"],
    items: [//                  {type:'checkbox',id:'currencyBtn',fn:'toggleCurrencyFormat',value:'',bIcon:'currencyIcon'},
    {
      type: 'checkbox',
      id: 'percentageBtn',
      fn: 'togglePercentageFormat',
      value: '',
      bIcon: 'percentageIcon'
    }, {
      type: 'checkbox',
      id: 'commaBtn',
      fn: 'toggleCommaFormat',
      value: '',
      bIcon: 'commaIcon'
    }, {
      type: 'action',
      id: 'increaseDecimalsBtn',
      fn: 'addDecimal',
      value: '',
      bIcon: 'increaseDecimalsIcon'
    }, {
      type: 'action',
      id: 'decreaseDecimalsBtn',
      fn: 'remDecimal',
      value: '',
      bIcon: 'decreaseDecimalsIcon'
    }]
  }], [{
    type: 'buttons',
    label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.currency"],
    items: [{
      type: 'dropdown',
      drop: true,
      id: 'currencyBtn1',
      ...
      options: {
        'none': {
          label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.currency.none"],
          value: '',
          fn: 'applyCurrencyFormat'
        },
        'locale_specific': {
          label: jive.i18n["net.sf.jasperreports.components.headertoolbar.label.localespecific"],
          value: "\xA4",
          fn: 'applyCurrencyFormat'
        },
        'dollar': {
  ...
  onCreate: function onCreate(jo) {
    var it = this,
        selector = $('#applyTo'); // events for applyTo selector
  ...
  updateApplyToSelector: function updateApplyToSelector(hideHeadings, isConditionalFormatting) {
    ...
    if (!hideHeadings && jive.selected.ie.config.headingsTabContent) {
      options.push({
        value: 'heading',
        text: jive.i18n["net.sf.jasperreports.components.headertoolbar.applyto.option.headings"]
      });
    }
    ...
    $.each(colGroups, function (i, group) {
      ...
      if ($.inArray(currentColIdx, group.forColumns) != -1) {
        ...
        if (group.groupType === 'groupheading') {
          option = {
            value: group.id,
            text: groupName + ' ' + jive.i18n["net.sf.jasperreports.components.headertoolbar.groupheading.prefix"]
          };
          ...
          option = {
            value: group.id,
            text: groupName + ' ' + jive.i18n["net.sf.jasperreports.components.headertoolbar.groupsubtotal.prefix"]
          };
          ...
          option = {
            value: group.id,
            text: jive.i18n["net.sf.jasperreports.components.headertoolbar.applyto.option.tabletotal"]
          };
    ...
    if (!isConditionalFormatting || isConditionalFormatting && jive.selected.ie.config.conditionalFormattingData) {
      extraOptions.push({
        value: 'detailrows',
        text: jive.i18n["net.sf.jasperreports.components.headertoolbar.applyto.option.detailrows"]
      });
    }
    ...
    $.each(finalOptions, function (i, v) {
      selector.append($("<option></option>", v));
    });
  ...
  onShow: function onShow() {
    ...
    jive.selected.form.jo.parent().css({
      'overflow-y': 'hidden'
    });
  ...
  columnChanged: function columnChanged() {
    ...
    !label.length && (label = "#" + (jive.selected.ie.config.columnIndex + 1)); // update dialog column name

    jive.ui.dialog.title.html(jive.i18n["net.sf.jasperreports.components.headertoolbar.title.formatcolumn"] + ': ' + label);
    this.updateColNavButtons();
    this.updateApplyToSelector();
  ...
  applyToChanged: function applyToChanged(val) {
    var it = this,
  ...
  updateColNavButtons: function updateColNavButtons() {
    var colIdx = jive.selected.ie.config.columnIndex,
        btnColNext = $("#colnext"),
        btnColPrev = $("#colprev");
  ...
  getNextVisibleColUuid: function getNextVisibleColUuid(colIdx) {
    var allColumns = jive.interactive.column.allColumns[jive.selected.ie.config.parentId],
        i;
  ...
  getPrevVisibleColUuid: function getPrevVisibleColUuid(colIdx) {
    var allColumns = jive.interactive.column.allColumns[jive.selected.ie.config.parentId],
        i;
  ...
  getCacheKey: function getCacheKey(prevApplyTo) {
  ...
  onGenericShow: function onGenericShow(applyToVal) {
    var metadata,
  ...
  getGroupMetadata: function getGroupMetadata(groupId) {
    var groupData = {};
...
jive.interactive.column.columnConditionalFormattingForm = {
  ...
  title: jive.i18n["net.sf.jasperreports.components.headertoolbar.title.conditions"],
  method: 'get',
  options: null,
  conditionType: null,
  prevApplyTo: null,
  templateElements: [{
    ...
    value: 'and',
    wrapClass: 't_wrap',
    tdClass: 'condition_between_start'
  }, {
    type: 'text',
    id: 'conditionEnd',
    value: '',
    ...
    items: [{
      type: 'checkbox',
      id: 'conditionFontBold',
      value: 'bold',
      bIcon: 'boldIcon',
      isTripleState: true
    }, {
      type: 'checkbox',
      id: 'conditionFontItalic',
      value: 'italic',
      bIcon: 'italicIcon',
      isTripleState: true
    }, {
      type: 'checkbox',
      id: 'conditionFontUnderline',
      value: 'underline',
      bIcon: 'underlineIcon',
      isTripleState: true
    }, {
      type: 'backcolor',
      id: 'conditionFontBackColor',
      bIcon: 'backgroundColorIcon',
      title: jive.i18n["net.sf.jasperreports.components.headertoolbar.title.fontbackcolor"],
      drop: true,
      showTransparent: true,
    ...
    items: [{
      ...
      type: 'action',
      id: 'conditionMoveDown',
      fn: 'conditionMoveDown',
      bIcon: 'moveDownIcon',
      btnClass: 'plain'
    }]
  }, {
    ...
    id: 'conditionRemove',
    bIcon: 'deleteIcon',
    fn: 'removeFormatCondition',
    btnClass: 'plain',
    tdClass: 'last'
  }],
  elements: [[[{
    type: 'label',
    value: jive.i18n["net.sf.jasperreports.components.headertoolbar.conditions.list"],
    align: 'left',
    wrapClass: 'wrapper label'
  }]], [[{
    type: 'label',
    value: '#',
    isHeader: true,
    ...
    type: 'label',
    value: jive.i18n["net.sf.jasperreports.components.headertoolbar.condition.operator"],
    isHeader: true,
    tdWidth: '26%',
    align: 'center'
  }, {
    type: 'label',
    value: jive.i18n["net.sf.jasperreports.components.headertoolbar.condition.condition"],
    colspan: 3,
    isHeader: true,
    tdWidth: '30%',
    align: 'center'
    ...
    type: 'label',
    value: jive.i18n["net.sf.jasperreports.components.headertoolbar.condition.format"],
    isHeader: true,
    tdWidth: '26%',
    align: 'center'
  }, {
    ...
    type: 'button',
    id: 'conditionAdd',
    bLabel: jive.i18n["net.sf.jasperreports.components.headertoolbar.condition.add"],
    fn: 'addFormatCondition',
    btnClass: 'plain',
    nowrap: true
  }, {
    type: 'label',
  ...
  onCreate: function onCreate(jo) {
    ...
    form.on('change', 'select[name=conditionTypeOperator]', function (evt) {
      ...
      if (self.val().indexOf('NULL') >= 0) {
        conditionStart.prop('disabled', true);
      } else {
        conditionStart.prop('disabled', false);
      }
    });
  ...
  onGenericShow: function onGenericShow(applyToVal) {
    ...
    if (metadata) {
      ...
      it.options = jive.interactive.column.operators[it.conditionType]; // enable Add button
      ...
      jive.selected.form.jo.find('table:eq(1) tr.jive_condition').each(function () {
        it.removeRow($(this));
      });
  ...
  onBlur: function onBlur(prevApplyTo) {
    var it = this,
        currentApplyTo = prevApplyTo || $('#applyTo').val(),
        cacheKey; // do not cache actionData for headings
  ...
  addFormatCondition: function addFormatCondition(jo, conditionData, isFromCache) {
    if (!jo.is('.disabled')) {
      var it = this,
  ...
  removeFormatCondition: function removeFormatCondition(jo) {
  ...
  removeRow: function removeRow(row) {
    var inputs = jive.selected.form.inputs;
    row.find('[name^=condition]').each(function () {
      delete inputs[$(this).attr('id')];
    });
    row.find('.jive_inputbutton[bname^=conditionFont]').each(function () {
      delete inputs[$(this).attr('bname')];
    });
    row.remove();
  },
  conditionMoveUp: function conditionMoveUp(jo) {
    var row = jo.closest('tr'),
        prev = row.prev(),
        table = row.closest('table');
  ...
  conditionMoveDown: function conditionMoveDown(jo) {
    var row = jo.closest('tr'),
        next = row.next(),
        table = row.closest('table'),
        rows = table.find('tr');
...
module.exports = jive.interactive.column;
