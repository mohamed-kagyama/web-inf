...
Map.prototype = {
  ...
  _init: function _init() {
    var it = this,
        ...
        reqParams = instData.requestParams || ""; // try to load the Gogle Maps API once, otherwise conflicts will happen
  ...
  _showMap: function _showMap(canvasId, latitude, longitude, zoom, mapType, markers, p) {
    ...
    if (markers) {
      ...
      for (var i = 0; i < markers.length; i++) {
        ...
        if (markerProps['icon.url'] && markerProps['icon.url'].length > 0) it._configureImage('icon', markerProps, markerOptions);else if (markerProps['icon'] && markerProps['icon'].length > 0) markerOptions['icon'] = markerProps['icon'];else if (markerProps['color'] && markerProps['color'].length > 0) {
          markerOptions['icon'] = 'https://chart.apis.google.com/chart?chst=d_map_pin_letter&chld=%7C' + markerProps['color'];
        }
        if (markerProps['shadow.url'] && markerProps['shadow.url'].length > 0) it._configureImage('shadow', markerProps, markerOptions);else if (markerProps['shadow'] && markerProps['shadow'].length > 0) markerOptions['shadow'] = markerProps['shadow'];
    ...
    if (p) {
      for (var k = 0; k < p.length; k++) {
        ...
        if (o['path.hyperlink']) {
          gg.event.addListener(poly, 'click', function () {
            window.open(o['path.hyperlink'], o['path.hyperlink.target']);
          });
        }
      }
    }
