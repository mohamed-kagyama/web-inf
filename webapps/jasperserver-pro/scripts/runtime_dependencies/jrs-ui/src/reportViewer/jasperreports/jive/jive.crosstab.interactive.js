...
var crosstabTemplate = require("text!./template/jive.crosstab.htm");

var i18n = require("bundle!jasperreports_messages");

require("csslink!./theme/jive.crosstab.csslink.css");
...
var ixt = {
    ...
    if (!self.initialized) {
      ...
      */
      self.reportInstance.on("beforeSearchAdvance", function (evt) {
        self.canFloat = false;
        self.hideFloatingElements(); // releasing this flag after 1.5 seconds
      ...
      self.reportInstance.on("beforeAction", function (evt) {
        // prevent floating
        self.canFloat = false; // prevent caching floating headers

        self.removeFloatingElements(); // hide column selection if visible

        self.active && self.hide();
      });
      $('#jivext_components').length == 0 && $('body').append('<div id="jivext_components"></div>');
      $('#jivext_components').empty();
      $('#jivext_components').append(_.template(crosstabTemplate));
  ...
  initCrosstab: function initCrosstab(crosstab) {
  ...
  selectRowGroup: function selectRowGroup(crosstab, cell, altCell) {
    var columnIdx = cell.data('jrxtcolidx');
    var fragmentId = cell.data('jrxtid');
  ...
  foobar: {
    ...
    setToolbarPosition: function setToolbarPosition() {
      ...
      it.jo.offset({
        top: top,
        left: ixt.selected.header.offset().left
      });
      it.jo.offset({
        top: top,
        left: ixt.selected.header.offset().left
      ...
      if (ixt.isFloatingColumnHeader) {
        ...
        this.jo.offset({
          top: ixt.selected.header.offset().top
        });
      }
    },
    render: function render(actionMap) {
      var it = this;
      var tmpl = ['<button class="jivext_foobar_button" title="',, '" actionkey="',, '" ',, '><span class="wrap"><span class="icon ',, '"></span></span></button>'];
  ...
  justHide: function justHide() {
    ixt.justHidden = true;
    ixt.overlay.jo.hide();
    ixt.foobar.jo.hide();
  },
  reApplySelection: function reApplySelection() {
    var it = this;
    it.justHidden = false;
  ...
  setScrollableHeader: function setScrollableHeader(isDashboard) {
    ...
    if (it.isIE) {
      ...
      $('body').on('scroll', function () {
        it.scrollHeaders(isDashboard); // reposition jive visual elements
  ...
  scrollHeaders: function scrollHeaders(isDashboard, forceScroll) {
    if (!this.canFloat) {
      return;
    }
    ...
    var it = this,
        ...
        scrolledTop = false,
        scrolledLeft = false; // Determine scroll direction
  ...
  hideFloatingElements: function hideFloatingElements() {
  ...
  removeFloatingElements: function removeFloatingElements() {
    var self = this;
  ...
  scrollColumnHeader: function scrollColumnHeader() {
    var it = this,
  ...
  scrollRowHeader: function scrollRowHeader() {
    var it = this,
        scrollContainer = this.$scrollContainer,
        crosstabId;
  ...
  scrollCrossSection: function scrollCrossSection() {
  ...
  getFloatingTable: function getFloatingTable(firstHeader, crosstabId, tableClass, elementClass, altElementClass) {
    var it = this,
    ...
    if (firstHeader && tbl.length === 0) {
        ...
        tbl.on("click", ".jrxtcrossheader", function (evt) {
          ...
          if (!$(evt.target).parent().is('._jrHyperLink')) {
            ...
            if (it.isFloatingRowHeader) {
              altJo = it.floatingRowHeader.find("td.jrxtrowheader[data-jrxtid='" + crosstabId + "']").filter("td[data-jrxtcolidx='" + colIdx + "']");
              jo = altJo.eq(0);
            } else {
              altJo = tbl.parent().find("table.jrPage td.jrxtrowheader[data-jrxtid='" + crosstabId + "']").filter("td[data-jrxtcolidx='" + colIdx + "']");
              jo = altJo.eq(0);
            }
      ...
      if (firstHeader.length > 0) {
        ...
        $.each(rows, function (idx, row) {
          ...
          appendedCloneTdCount = 0; // add only the tds with elementClass class
  ...
  getColumnHeaderFloatingTable: function getColumnHeaderFloatingTable(firstHeader, crosstabId) {
  ...
  setToolbarPositionWhenFloating: function setToolbarPositionWhenFloating(isActive, isDashboard) {
    var it = this,
        top,
        firstHeader,
        toolbarTop,
  ...
  markCrossHeaderElements: function markCrossHeaderElements(crosstabId) {
