...
var orgModule = require("./org.role.mng.main");
...
orgModule.roleManager.properties = {
  ...
  initialize: function initialize() {
    ...
    this.roleName.unsupportedSymbols = new RegExpRepresenter(orgModule.Configuration.roleNameNotSupportedSymbols).getRepresentedString();
    this.roleName.inputValidator = orgModule.createInputRegExValidator(this.roleName);
    this._validators = [orgModule.createBlankValidator(this.roleName, "roleNameIsEmpty"), this.roleName.inputValidator];
    ...
    orgModule.properties._deleteEntity = function () {
      orgModule.invokeClientAction(orgModule.ActionMap.DELETE, {
        entity: this._value
      });
    };
    ...
    orgModule.properties.isChanged = function () {
      ...
      return this.isEditMode && (oldRole.roleName != role.roleName || this.getAssignedEntities().length > 0 || this.getUnassignedEntities().length > 0);
    };
  ...
  _toRole: function _toRole(tenantId) {
    var panel = $(orgModule.properties._id);
...
orgModule.roleManager.addDialog = {
  ...
  show: function show(organization) {
    ...
    if (title) {
      var msg = this.organization && !this.organization.isRoot() ? orgModule.getMessage('addRoleTo', {
        organizationName: orgModule.truncateOrgName(this.organization.id)
      }) : orgModule.getMessage('addRole');
      title.update(msg);
    }

    this.addBtn.title = this.organization && !this.organization.isRoot() ? orgModule.getMessage('addRoleTo', {
  ...
  _doAdd: function _doAdd() {
    if (this._validate()) {
      ...
      if (this.organization && !this.organization.isRoot()) {
        role.tenantId = this.organization.id;
      }
