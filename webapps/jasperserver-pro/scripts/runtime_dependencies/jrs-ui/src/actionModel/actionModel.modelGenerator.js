...
var fitObjectIntoScreen = _utilUtilsCommon.fitObjectIntoScreen;
var getAsFunction = _utilUtilsCommon.getAsFunction;
var getWindowHeight = _utilUtilsCommon.getWindowHeight;
var getWindowWidth = _utilUtilsCommon.getWindowWidth;
...
actionModel.MOUSE_OUT_PATTERNS = ['#' + layoutModule.MENU_ID, '.' + layoutModule.MENU_ROOT_CLASS, '.toolbar .capsule', '.header > .button.mutton', '#' + layoutModule.MENU_ID + ' *', layoutModule.MENU_ROOT_CLASS + ' *']; ////////////////////////////////////////////////////////////////////
// Menu building
////////////////////////////////////////////////////////////////////
...
actionModel.showDynamicMenu = function (menuContext, event, className, coordinates, actionModelScriptTag, updateContextActionModel) {
  ...
  actionModel.adjustMenuPosition(actionModel.menuDom, menuContext); //actionModel.focusMenu();

  if (event && event.type !== 'click' && menuContext !== "folder_mutton") {
    // avoid attaching the mouseout close handler if menu is opened by mouse click
...
actionModel.closeHandler = function (event) {
  ...
  if ((target.hasClass('mutton') || target.parents('.mutton').length) && event.relatedTarget || target.attr('id') === 'menu' || target.parents('#menu').length) {} //|| actionModel.lastInput=="key"
...
actionModel.resetMenu = function (menuContext) {
  ...
  menuList.writeAttribute("js-navtype", dom.getAttribute("js-navtype")); //remove any generated styles.
...
actionModel.appendDesiredRowType = function (thisAction, event, mouseUpFunction, contentParent) {
  var mouseUpFunctionWithMenuHiding = function mouseUpFunctionWithMenuHiding() {
...
actionModel.addSelector = function (thisAction, contentParent, event) {
  ...
  if (className == 'flyout') {
    ...
    if (thisAction && thisAction.children.length > 0) {
      ...
      actionModel.insertMenuRow(contentParent, parentSelector, false, thisAction.isDisabled, thisAction.id, null); //now create submenu
      //now create submenu

      var submenu = actionModel.menuDom.cloneNode(false);
      submenu.writeAttribute('id', parentSelector.readAttribute('id') + '_subMenu'); //add common template
      //add common template
...
actionModel.showChildSubmenu = function (parent) {
  ...
  */
  ...
  if (submenu) {
    submenu.setStyle({
      display: actionModel.DISPLAY_STYLE
    });
    submenu.setStyle({
      position: 'absolute'
    });
    ...
    submenuHeight = $(submenu).getHeight(); //hack to prevent  bug20863  (Due to Tim's markup change)
...
actionModel.makeMenuVisible = function (menu) {
  ...
  if (list.childElements().length > 0) {
    menu.setStyle({
      zIndex: actionModel.HIGH_Z_INDEX
    });
...
actionModel.focusMenu = function () {
  ...
  if (!jQuery.contains(jQMenuDom, document.activeElement)) {
    ...
    actionModel.lastFocused = document.activeElement; //!menuDom.hasClassName(layoutModule.HIDDEN_CLASS) && menuDom.focus();
...
actionModel.hideMenu = function () {
  if (actionModel.isMenuShowing()) {
    var setNextFocus = function setNextFocus() {
      var nextFocus = actionModel.lastFocused;

      try {
        nextFocus && nextFocus.focus();
      } catch (ex) {} //IE gets bothered if you try to focus something that it can't
...
function fireMenuActionWithEvent(args, funcString, event) {
  var func = toFunction(funcString),
      belongsToLocalContext;
