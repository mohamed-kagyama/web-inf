...
var logger = log.register("stdnav"),
    singleton = null,
    gserial = 0; // ===EXTERNAL SYMBOLS USED:=============================================
// layoutModule
// Local object definition.

var stdnavPluginActionMenu = function stdnavPluginActionMenu() {
  gserial++;
  this.serial = gserial;
...
_.extend(stdnavPluginActionMenu.prototype, {
  ...
  _onFocusIn: function _onFocusIn(element) {
    ...
    if ($selected.length > 0) {
      ...
      if ($thisItem.length > 0) {
        ...
        buttonManager.over($thisItem.find(layoutModule.BUTTON_PATTERN)[0]); //buttonManager.over($thisItem.find(layoutModule.MENU_LIST_PATTERN)[0]);
  ...
  _onSuperfocusOut: function _onSuperfocusOut(element) {
    ...
    if (newFocus.closest('.dropDown,.context').length < 1) {
      ...
      actionModel.hideMenu(); // Explicitly make the element unfocusable-- temporarily.
  ...
  /* ====== MENU BEHAVIOR ====== */
  _focus_prev_menu_entry: function _focus_prev_menu_entry(entry) {
    var newFocus;
    var wasOpen = false;
    ...
    if (wasOpen === true) {//this._open_submenu(newFocus.children('.menu'));
    }
  },
  _focus_next_menu_entry: function _focus_next_menu_entry(entry) {
    var newFocus;
    var wasOpen = false;
  ...
  _onSubfocusIn: function _onSubfocusIn(element) {
    ...
    if ($(element).prop('nodeName') === 'li' === false) {
      ...
      var subel = this._fixFocus(element); // Adjust subfocus without firing callbacks.
  ...
  */
  ...
  _onUp: function _onUp(element) {
    var $prev = $(element); // Figure out whether we're in the main menu or the popup.
    ...
    if ($menu.length > 0) {
      ...
      $prev = $thisItem.prev(layoutModule.MENU_LIST_PATTERN); // Oddly, trying to add ":not(.separator)" to the pattern
  ...
  _onDown: function _onDown(element) {
    var $next = $(element); // Figure out whether we're in the main menu or the popup.
    ...
    if ($menu.length > 0) {
      // We're in a drop-down or context menu.
      var $thisItem = $(document.activeElement).closest(layoutModule.MENU_LIST_PATTERN);
      $next = $thisItem.next(layoutModule.MENU_LIST_PATTERN); // Oddly, trying to add ":not(.separator)" to the pattern
      // above did not work; the separator was indeed skipped, but
      // the menu item after it was not returned.  This code skips
      // over any number of adjacent separators.
...
module.exports = new stdnavPluginActionMenu();
