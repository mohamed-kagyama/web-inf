...
(function (exp, _, Controls) {
  var DataConverter = Controls.Base.extend({
    ...
    structureFormater: function structureFormater(response) {
      if (!response) {
        return response;
      }
      ...
      }, this.convertResponseToControlsState(state));
    },
    //Convert controls update response to data structure compatible with controls
    convertResponseToControlsState: function convertResponseToControlsState(rawState) {
      var result = {};
      var requestStates = rawState.inputControlState ? rawState.inputControlState : rawState;

      var convertOptionFunc = function convertOptionFunc(option) {
        var opt = {
          value: option.value,
          label: option.label
        };
      ...
      var convertOptionsFunc = function convertOptionsFunc(options) {
        var values;
      ...
      var convertToValues = function convertToValues(requestState) {
