...
var InputControlsDataProvider = require('./dataprovider/InputControlsDataProvider');

var InputControlsDataProviderWithDataLabelHash = require('./dataprovider/InputControlsDataProviderWithDataLabelHash');

var DataProviderWithSearchCache = require("runtime_dependencies/js-sdk/src/common/component/tree/dataprovider/DataProviderWithSearchCache");

var getInputControlsDataProviderNewPageOptions = require('./dataprovider/util/getInputControlsDataProviderNewPageOptions');
...
var getInitialSelectedValuesToLabelMap = function getInitialSelectedValuesToLabelMap(initialSelectedValues) {
  return initialSelectedValues.reduce(function (memo, selectedValue) {
...
JRS.Controls = function (jQuery, _, Controls) {
  ...
  function changeDateFunc(dateText) {
    var prevSelection = Controls.BaseControl.prototype.get.call(this, 'selection');
  ...
  return _.extend(Controls, {
    Bool: Controls.BaseControl.extend({
      setValue: function setValue(controlData) {
    ...
    SingleSelect: Controls.BaseControl.extend({
      initialize: function initialize(controlStructure) {
        var _this = this;

        var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
        var initialSelectedValues = options.initialSelectedValues;
        ...
        if (!this.singleSelect) {
          ...
          this.dataProviderWithCache = new DataProviderWithSearchCache({
            ...
            request: function request(options) {
              options = getInputControlsDataProviderNewPageOptions(_extends({
                name: _this.id,
                paginatedValuesOptions: _this.paginatedValuesOptions
              }, options));

              var dfd = _this.inputControlsDataProviderWithDataLabelHash.getData(_this.dataUri, options);
          ...
          this.singleSelect = new SingleSelect({
            ...
            formatValue: function formatValue(value) {
              return _this.inputControlsDataProviderWithDataLabelHash.getLabelByValue(value) || _this.initialSelectedValuesToLabelMap[value];
            }
      ...
      setValue: function setValue(selectedValues) {
        this.singleSelect.setValue(selectedValues, {
          silent: true
        });
      },
      updateSelectionOnOptionChange: function updateSelectionOnOptionChange(selection) {
        this.initialSelectedValuesToLabelMap = getInitialSelectedValuesToLabelMap(selection);
      },
      fetch: function fetch(uri, paginatedValuesOptions) {
        var _this2 = this;

        this.dataUri = uri;
        this.paginatedValuesOptions = paginatedValuesOptions;
        var dfd = jQuery.Deferred();
        this.singleSelect.off('listRenderError').once('listRenderError', function (xhr) {
          showErrorPopup(xhr.responseJSON.message);
          dfd.reject(xhr);

          _this2.bindCustomEventListeners();
        });
      ...
      bindCustomEventListeners: function bindCustomEventListeners() {
        ...
        this.singleSelect.off('listRenderError').on('listRenderError', function (xhr) {
          showErrorPopup(xhr.responseJSON.message);
        });
      }
    }),
    MultiSelect: Controls.BaseControl.extend({
        ...
        if (!this.multiSelect) {
          ...
          this.dataProviderWithCache = new DataProviderWithSearchCache({
            ...
            request: function request(options) {
              options = getInputControlsDataProviderNewPageOptions(_extends({
                name: _this3.id,
                paginatedValuesOptions: _this3.paginatedValuesOptions
              }, options));

              var dfd = _this3.inputControlsDataProviderWithDataLabelHash.getData(_this3.dataUri, options);
          ...
          this.multiSelect = new MultiSelect({
            ...
            selectedListOptions: {
              formatLabel: function formatLabel(value) {
                return _this3.inputControlsDataProviderWithDataLabelHash.getLabelByValue(value) || _this3.initialSelectedValuesToLabelMap[value];
              },
      ...
      setValue: function setValue(selectedValues) {
        this.multiSelect.setValue(selectedValues, {
          silent: true
        });
      ...
      fetch: function fetch(uri, paginatedValuesOptions) {
        ...
        this.paginatedValuesOptions = paginatedValuesOptions;
        this.dataProviderWithCache.clear();
        this.multiSelect.off('listRenderError').once('listRenderError', function (xhr) {
          showErrorPopup(xhr.responseJSON.message);
          dfd.reject(xhr);

          _this4.bindCustomEventListeners();
        });
      ...
      bindCustomEventListeners: function bindCustomEventListeners() {
        ...
        this.multiSelect.off('listRenderError').on('listRenderError', function (xhr) {
          showErrorPopup(xhr.responseJSON.message);
        });
      },
      get: function get(attribute) {
    ...
    SingleSelectRadio: Controls.BaseControl.extend({
      ...
      fetch: function fetch(uri, paginatedValuesOptions) {
        ...
        return this._fetchData(uri, paginatedValuesOptions).then(function (result) {
          _this5._renderData(result.data);
        });
      },
      _fetchData: function _fetchData(uri, paginatedValuesOptions) {
        this.dataUri = uri;
        this.paginatedValuesOptions = paginatedValuesOptions;
        var options = getInputControlsDataProviderNewPageOptions({
          name: this.id,
      ...
      _renderData: function _renderData(controlData) {
        var _this6 = this;

        var list = this.getElem() && this.getElem().find('ul')[0];

        if (list) {
          var data = controlData.map(function (val) {
            var result = _.clone(val);

            result.readOnly = _this6.readOnly;
            result.name = _this6.getOptionName();
            result.uuid = _.uniqueId(_this6.id);
            return result;
          });
      ...
      bindCustomEventListeners: function bindCustomEventListeners() {
        ...
        this.getElem().on('change', 'input', function (evt) {
    ...
    MultiSelectCheckbox: Controls.BaseControl.extend({
      ...
      fetch: function fetch(uri, paginatedValuesOptions) {
        ...
        return this._fetchData(uri, paginatedValuesOptions).then(function (result) {
          _this8._renderData(result.data);
        });
      },
      _fetchData: function _fetchData(uri, paginatedValuesOptions) {
        this.dataUri = uri;
      ...
      _renderData: function _renderData(controlData) {
        ...
        var data = controlData.map(function (val) {
          ...
          result.readOnly = _this9.readOnly;
          result.uuid = _.uniqueId(_this9.id);
          return result;
        });
        var list = this.getElem() && this.getElem().find('ul')[0];
      ...
      bindCustomEventListeners: function bindCustomEventListeners() {
        ...
        this.getElem().on('change', 'input', function (evt) {
          var selection = _this10.getSelection(); // for better performance in IE7
        ...
        this.getElem().on('click', 'a', function (evt) {
          var options = _this10.getElem().find('input');
          ...
          _this10.getElem().find('input').change(); // trigger the cascading request
