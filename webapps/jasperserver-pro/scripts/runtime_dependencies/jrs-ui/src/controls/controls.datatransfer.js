...
require('./controls.dataconverter');
...
JRS.Controls = function (JSON, jQuery, _, Controls) {
  ...
  function commonErrorHandler(err) {
    ...
    try {
      ...
      if (errorObject && errorObject.error) {
        ...
        } else if ([400, 500].indexOf(err.status) >= 0 || err.getResponseHeader("JasperServerError") && !err.getResponseHeader("SuppressError")) {
          var message = "";
  ...
  return _.extend(Controls, {
    ...
    DataTransfer: Controls.Base.extend({
      ...
      setReportUri: function setReportUri(reportUri) {
        this.currentReportUri = reportUri;
      },
      buildUrl: function buildUrl(reportPath, controlIds, urlTemplate) {
       ...
       * @returns {*}
       */
      sendRequest: function sendRequest(url, data, settings) {
        var defaultSettings = {
          url: url,
          type: "GET",
          dataType: "json",
        ...
        return jQuery.ajax(requestSettings).fail(commonErrorHandler);
      },
      fetchControlsStructure: function fetchControlsStructure(uris, selectedData) {
      ...
      updateControlsStructure: function updateControlsStructure(updatedStructure, uri) {
        var url = this.buildUrl(uri || this.currentReportUri, null, this.CONTROLS_STRUCTURE_TEMPLATE_URI);
        return this.sendRequest(url, updatedStructure, {
          type: "PUT"
        }).then(_.bind(this.dataConverter.structureFormater, this.dataConverter));
      },
      fetchInitialControlValues: function fetchInitialControlValues(reportUri, selectedData) {
        var url = this.buildUrl(reportUri, null, this.INITIAL_CONTROLS_VALUES_TEMPLATE_URL);
        var initialControlsValues = this.sendRequest(url, selectedData).then(this.dataConverter.convertResponseToControlsState);
        Controls.Utils.showLoadingDialogOn(initialControlsValues, null, true);
        return initialControlsValues;
      },
      fetchControlsUpdatedValues: function fetchControlsUpdatedValues(ids, selectedData) {
        ...
        triggerRequestDeferred.done(_.bind(function () {
          this.sendRequest(url, selectedData).then(dataConverter.convertResponseToControlsState).done(function (data) {
