...
orgModule.manager = {
  initialize: function initialize(options) {
    ...
    if (isProVersion()) {
      this.tenantsTree = new options.TenantsTreeView({
        ...
        comparator: function comparator(t1, t2) {
    ...
    orgModule.observe("org:browse", function (event) {
      ...
      if (force || !this.lastSelectedOrg || this.lastSelectedOrg.id != org.id) {
        ...
        if (orgModule.invokeClientAction("cancelIfEdit", {
          entity: entities[0] || org,
    ...
    }.bindAsEventListener(this)); // Response events listeners.
  ...
  updateEntityList: function updateEntityList(entityJson, entityName) {
  ...
  entityJsonToObject: function entityJsonToObject(json) {// Template method.
  },
  relatedEntityJsonToObject: function relatedEntityJsonToObject(json) {// Template method.
  },
  reloadEntities: function reloadEntities() {
    orgModule.fire(orgModule.Event.ORG_BROWSE, {});
  },
  isUserSuperuser: function isUserSuperuser() {
    var superuserRole = isProVersion() ? orgModule.Configuration.superuserRole : orgModule.Configuration.adminRole;
    var roles = this.options.currentUserRoles;
    return roles.detect(function (r) {
      return r.roleName == superuserRole && !r.tenantId;
    }) != null;
  }
};
