...
var dynamicTree = require('./dynamicTree.treenode');

var layoutModule = require('../core/core.layout');

var designerBase = require('./dynamicTree.designerBase');
...
/* global alert*/
dynamicTree.Tree.addVar('NODE_PATTERN', ".node"). //node itself
addVar('NODE_WRAPPER_PATTERN', ".node > .wrap").addVar('NODE_ICON_PATTERN', ".node > .wrap .icon").addVar('NODE_CUSTOM_PATTERNS', []);
dynamicTree.Tree.addVar('LEAF_PATTERN', ".leaf"). //leaf itself
addVar('LEAF_WRAPPER_PATTERN', ".leaf > .wrap").addVar('LEAF_ICON_PATTERN', ".leaf > .wrap .icon").addVar('LEAF_CUSTOM_PATTERNS', []);
...
dynamicTree.Tree.addMethod('_registerClickEvents', function () {
  ...
  treeContainer.observe('click', function (event) {
    ...
    if (isNode) {
      isIcon && treeContainer.fire('nodeIcon:click', {
        targetEvent: event,
        node: node
      });
      treeContainer.fire('node:click', {
        targetEvent: event,
        node: node
      });
    } else if (isLeaf) {
  ...
  treeContainer.observe('dblclick', function (event) {
    ...
    if (isNode) {
      isIcon && treeContainer.fire('nodeIcon:dblclick', {
        targetEvent: event,
        node: node
      });
      treeContainer.fire('node:dblclick', {
        targetEvent: event,
        node: node
      });
    } else if (isLeaf) {
      isIcon && treeContainer.fire('leafIcon:dblclick', {
        targetEvent: event,
        node: node
      });
      treeContainer.fire('leaf:dblclick', {
        targetEvent: event,
        node: node
      });
    }
...
dynamicTree.Tree.addMethod('_registerMouseEvents', function () {
  ...
  treeContainer.observe(isSupportsTouch() ? 'touchstart' : 'mousedown', function (event) {
    ...
    if (!isSupportsTouch() || !JRS.vars.ajax_in_progress) {
      ...
      if (isNode) {
        isIcon && treeContainer.fire('nodeIcon:mousedown', {
          targetEvent: event,
          node: node
        });
        treeContainer.fire('node:mousedown', {
          targetEvent: event,
          node: node
        });
      } else if (isLeaf) {
        isIcon && treeContainer.fire('leafIcon:mousedown', {
          targetEvent: event,
          node: node
        });
        treeContainer.fire('leaf:mousedown', {
          targetEvent: event,
          node: node
        });
      }
    }
  }.bindAsEventListener(this)); //scriptaculous stopped mousedown event but we made it throw this instead
  ...
  treeContainer.observe(isSupportsTouch() ? 'drag:touchstart' : 'drag:mousedown', function (e) {
    ...
    event.treeEvent = true;
    var isIcon = this.isIconEvent(event),
        isNode = this.isNodeEvent(event),
        isLeaf = this.isLeafEvent(event);
    ...
    if (isNode) {
      isIcon && treeContainer.fire('nodeIcon.drag:mousedown', {
        targetEvent: event,
        node: node
      });
      treeContainer.fire('node.drag:mousedown', {
        targetEvent: event,
        node: node
      });
    } else if (isLeaf) {
      isIcon && treeContainer.fire('leafIcon.drag:mousedown', {
        targetEvent: event,
        node: node
      });
      treeContainer.fire('leaf.drag:mousedown', {
        targetEvent: event,
        node: node
      });
    }
  ...
  treeContainer.observe(isSupportsTouch() ? 'touchend' : 'mouseup', function (event) {
    ...
    if (!isSupportsTouch() || !JRS.vars.ajax_in_progress) {
      ...
      if (isNode) {
        isIcon && treeContainer.fire('nodeIcon:mouseup', {
          targetEvent: event,
          node: node
        });
        treeContainer.fire('node:mouseup', {
          targetEvent: event,
          node: node
        });
      } else if (isLeaf) {
        isIcon && treeContainer.fire('leafIcon:mouseup', {
          targetEvent: event,
          node: node
        });
        treeContainer.fire('leaf:mouseup', {
          targetEvent: event,
          node: node
        });
      }
  ...
  if ('createTouch' in document) {
    ...
    treeContainer.observe('mouseover', function (event) {
      ...
      if (Draggables.dragging) {
        ...
        this.timeout_id = setTimeout(function (event) {
          node.openNode(event);
        }, this.EXPANDING_TIME);
      }

      this.createDraggableIfNeeded(event, node);
      !isIE7() && treeContainer.fire('tree:mouseover', {
        targetEvent: event,
        node: node
      });
    }.bindAsEventListener(this));
...
dynamicTree.Tree.addMethod('_registerCustomEvents', function () {
  ...
  treeContainer.observe('mousedown', function (event) {
    if (!dynamicTree.activeTreeId || dynamicTree.activeTreeId !== this.getId()) {
      treeContainer.fire('tree:blur', {
        targetEvent: event,
        tree: dynamicTree.getActiveTree()
      });
      dynamicTree.activeTreeId = this.getId();
      treeContainer.fire('tree:focus', {
        targetEvent: event,
        tree: dynamicTree.getActiveTree()
      });
    }
  ...
  treeContainer.observe('mouseover', function (event) {
    ...
    if (!node) {
      treeContainer.fire('tree:mouseover', {
        targetEvent: event,
        tree: this
      });
    }
...
dynamicTree.Tree.addMethod('fireSelectEvent', function (node, event) {
  ...
  treeContainer.fire(node.isParent() ? 'node:selected' : 'leaf:selected', {
    node: node,
    targetEvent: event
  });
});
dynamicTree.Tree.addMethod('fireUnSelectEvent', function (node, event) {
  ...
  treeContainer.fire(node.isParent() ? 'node:unselected' : 'leaf:unselected', {
    node: node,
    targetEvent: event
  });
});
dynamicTree.Tree.addMethod('fireUnSelectAllEvent', function (event) {
  ...
  treeContainer.fire('items:unselected', {
    targetEvent: event
  });
});
dynamicTree.Tree.addMethod('fireEditEvent', function (node, newVal) {
  ...
  treeContainer.fire(node.isParent() ? 'node:edit' : 'leaf:edit', {
    node: node,
    newValue: newVal
  });
});
dynamicTree.Tree.addMethod('fireStartEditEvent', function (node, input) {
  ...
  treeContainer.fire(node.isParent() ? 'node:startEdit' : 'leaf:startEdit', {
    node: node,
    input: input
  });
});
dynamicTree.Tree.addMethod('fireEndEditEvent', function (node) {
  ...
  treeContainer.fire(node.isParent() ? 'node:endEdit' : 'leaf:endEdit', {
    node: node
  });
});
/**
  ...
  */
  ...
  if (this.dragPattern && !isCreated) {
    ...
    if (matchingElem && thisElem.className.toLowerCase().indexOf('icon') < 0 && !this.draggables[matchingElem.identify()]) {
      this.draggables[matchingElem.identify()] = new Draggable(thisElem, {
        superghosting: true,
        mouseOffset: true,
...
dynamicTree.Tree.addMethod('setDragStartState', function (node, draggable, event) {
  ...
  draggable.element.addClassName(layoutModule.DRAGGING_CLASS).addClassName(this.getId()); // Customize draggable style
  ...
  if (this.selectedNodes.length > 1) {
    draggable.element.update(new Template(this.TREE_NN_ITEMS_SELECTED).evaluate({
