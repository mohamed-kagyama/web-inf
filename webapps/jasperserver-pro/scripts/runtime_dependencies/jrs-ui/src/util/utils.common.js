...
var utilsCommonEnum = require('./utils.common.enum');

var Builder = require('scriptaculous/src/builder');

var _toolsDrag = require('./tools.drag');

var Dragger = _toolsDrag.Dragger;
var DragListener = _toolsDrag.DragListener;
...
var $w = _prototype.$w;
var $$ = _prototype.$$;
var Prototype = _prototype.Prototype;
...
function doNothing() {} //what it says
...
function addDataToForm(form, postData) {
  var addHiddenInputToForm = function addHiddenInputToForm(parameterName, parameterValue) {
...
function getElementWithIdAndTagAndParent(id, tagName, parentElem) {
  ...
  for (var i = 0; i < elems.length; i++) {
    if (elems[i].getAttribute('id') === id) return elems[i];
  }
...
function isDisabled(elem) {
  ...
  var hasAttribute = !!elem.readAttribute('disabled') || elem.ancestors().any(function (s) {
    return s.readAttribute('disabled');
...
function getBufferHeight(elem, includeMargins) {
  ...
  result += new Number(elem.getStyle('borderBottomWidth').replace(digitRegex, '')).ceil() + new Number(elem.getStyle('borderTopWidth').replace(digitRegex, '')).ceil() + new Number(elem.getStyle('paddingTop').replace(digitRegex, '')).ceil() + new Number(elem.getStyle('paddingBottom').replace(digitRegex, '')).ceil();
  return result;
}
...
function getBufferWidth(elem, includeMargins) {
  ...
  result += new Number(elem.getStyle('borderLeftWidth').replace(digitRegex, '')).ceil() + new Number(elem.getStyle('borderRightWidth').replace(digitRegex, '')).ceil() + new Number(elem.getStyle('paddingLeft').replace(digitRegex, '')).ceil() + new Number(elem.getStyle('paddingRight').replace(digitRegex, '')).ceil();
  return result;
}
/**
...
function centerElement(elem, options) {
  ...
  if (options.horz) {
    ...
    if (w) {
      ...
      var parentWidth = elem.up().getWidth(); //if >= parent width nothing to do
  ...
  if (options.vert) {
    ...
    if (h) {
      ...
      var parentHeight = elem.up().getHeight(); //if >= parent height nothing to do
...
function disableSelection(target) {
  if (target) {
    if (typeof target.style.KhtmlUserSelect != 'undefined') {
      ...
      target.style.MozUserSelect = '-moz-none';else //All other route (ie: Opera)
...
function enableSelection(target) {
  if (target) {
    ...
    } else {} //All other route (e.g. Opera)
...
function getCellAtColumn(cells, columnIndex) {
  ...
  for (var i = 0; i < cells.length; i++) {
    ...
    if (columnCount >= columnIndex) return cells[i];
  }
...
function scrollXToMiddle(theBody) {
  ...
  if (excess > 0) theBody.scrollLeft = excess / 2;
} ///////////////////////////////////////////
// Window Creation
///////////////////////////////////////////
///////////////////////////////////////////
...
function getTextAfterSubstring(theText, theSubstring) {
  ...
  if (ssIndex == -1) return null;else return theText.substring(ssIndex + theSubstring.length);
}

function getTextBeforeSubstring(theText, theSubstring) {
  var ssIndex = theText.toLowerCase().indexOf(theSubstring.toLowerCase());
  if (ssIndex == -1) return theText;else return theText.substring(0, ssIndex);
}
...
function formatString(string) {
  var values = Array.prototype.slice.call(arguments, 1),
      ...
      parts,
      result = [],
      val; // Simple implementation for number of substitute strings less than 3
...
var inspect = function inspect(myObj) {
  var properties = [];
...
var getAsFunction = function getAsFunction(functionName) {
...
InPlaceEditor.prototype.makeEditable = function (options) {
  ...
  if (options) {
    ...
    if (options['onMouseup']) {
      var mouseHandler = function mouseHandler(evt) {
        evt = evt ? evt : window.event;
...
var tracer = {
  ...
  indentCount: -4,
  tracing: [],
  traceMe: function traceMe(func, methodName) {
    var traceOn = function traceOn() {
  ...
  traceAll: function traceAll(root, recurse) {
    if (root == window || !(_typeof(root) == 'object' || typeof root == 'function')) {
      return;
    }
  ...
  untraceAll: function untraceAll() {
...
var ValidationModule = {
   ...
   */
  validate: function validate(validationEntries, showError) {
    var that = this;
   ...
   */
  validateEntry: function validateEntry(validationEntry, showError) {
   ...
   */
  _validateElement: function _validateElement(element, entry, showError) {
    var errorMessage;
  ...
  _runValidatorOnElement: function _runValidatorOnElement(validator, element, messages, options) {
   ...
   */
  attachOnEvent: function attachOnEvent(eventName, validationEntries) {
   ...
   */
  _getMessage: function _getMessage(key, customMessages, defaultMessages, values) {
   ...
   * @param values
   */
  fillPlaceholders: function fillPlaceholders(string, values) {
  ...
  methods: {
    ...
    mandatory: function mandatory(value, messages) {
    ...
    minMax: function minMax(value, messages, options) {
    ...
    minMaxLength: function minMaxLength(value, messages, options) {
    ...
    resourceIdChars: function resourceIdChars(value, messages) {
    ...
    wordChars: function wordChars(value, messages) {
      if (value.search(/\W/) >= 0) {
        return this._getMessage('wordInvalidChars', messages);
      }
    },
    startsWithLetter: function startsWithLetter(value, messages) {
   ...
   *  </ul>
   */
  validateLegacy: function validateLegacy(validationEntries, showError) {
    var isValid = true,
        elemsWithErrors = [];
   ...
   */
  _showMessage: function _showMessage(element, message, containerClass, messageTypeClass) {
    var msgContainer = element.validatorMessageContainer || element.parentNode;
    var msg = $(msgContainer).select(messageTypeClass)[0];
    msg = msg || $(msgContainer).select(utilsCommonEnum.MESSAGE_PATTERN)[0];
   ...
   */
  showError: function showError(element, errorMessage, detailsMessage) {
    this._showMessage(element, errorMessage, utilsCommonEnum.ERROR_CLASS, utilsCommonEnum.MESSAGE_WARNING_PATTERN);

    var msgContainer = element.validatorMessageContainer || element.parentNode;

    if (detailsMessage) {
      var detailsBtn = $(msgContainer).select(utilsCommonEnum.DETAILS_PATTERN)[0];
      ...
      var showDetails = $(msgContainer).select(utilsCommonEnum.MESSAGE_WARNING_DETAILS_PATTERN)[0];
      if (showDetails) $(showDetails).hide();
    }
  },
   ...
   */
  showSuccess: function showSuccess(element, successMessage) {
   ...
   */
  hideError: function hideError(element, cleanMessage) {
    var msgContainer = element.validatorMessageContainer || element.parentNode; // removing error class name
    // removing error class name

    $(msgContainer).removeClassName(utilsCommonEnum.ERROR_CLASS); // in some cases it's better to remove error message to prevent some errors.
    // in some cases it's better to remove error message to prevent some errors.

    if (cleanMessage) {
      // find element which has message
      var msg = $(msgContainer).select(utilsCommonEnum.MESSAGE_WARNING_PATTERN)[0] || $(msgContainer).select(utilsCommonEnum.MESSAGE_PATTERN)[0];
   ...
   * @param element the form element.
   */
  hideSuccess: function hideSuccess(element) {
    var msgContainer = element.validatorMessageContainer || element.parentNode;
    $(msgContainer).removeClassName(utilsCommonEnum.SUCCESS_CLASS);
  }
}; ///////////////////////////////////////////////
...
var ClassNameBuilder = function ClassNameBuilder(element) {
...
var JSCookie = function JSCookie(name, value, days) {
  if (!Object.isString(name)) {
    throw 'Invalid name';
  }
...
var sessionManager = {
  ...
  resetSession: function resetSession(flowExecutionKey) {
...
var buildActionUrl = function buildActionUrl(params) {
  if (!params) {
    return;
  }
...
var compileTemplate = function compileTemplate(templateSelector) {
  if (!templateSelector) {
    return;
...
window.isIE = isIE; // JRS-20335 formatString is used from global scope in WEB-INF/jsp/jsTemplates/tableRows.jsp
...
exports.isOS = isOS;
exports.isSupportsTouch = isSupportsTouch;
exports.isMozilla = isMozilla;
exports.isFirefox = isFirefox;
exports.isWebKitEngine = isWebKitEngine;
...
exports.isIEVersion7Upwards = isIEVersion7Upwards;
exports.isIEVersion = isIEVersion;
exports.getIEVersion = getIEVersion;
exports.doNothing = doNothing;
exports.addDataToForm = addDataToForm;
exports.getTZOffset = getTZOffset;
exports.exists = exists;
exports.isArray = isArray;
exports.isNumber = isNumber;
exports.isNotNullORUndefined = isNotNullORUndefined;
exports.hasDisabledAttributeSet = hasDisabledAttributeSet;
exports.getParentDiv = getParentDiv;
exports.getAbsoluteParent = getAbsoluteParent;
exports.getElementWithIdAndTagAndParent = getElementWithIdAndTagAndParent;
exports.getClosestAncestor = getClosestAncestor;
exports.isDescendantOf = isDescendantOf;
exports.getCellIndex = getCellIndex;
exports.getRowIndex = getRowIndex;
exports.isDisabled = isDisabled;
exports.matchAny = matchAny;
exports.matchMeOrUp = matchMeOrUp;
exports.stylesMatch = stylesMatch;
exports.getNextMatchingSibling = getNextMatchingSibling;
exports.getNextNonMatchingSibling = getNextNonMatchingSibling;
exports.getPreviousMatchingSibling = getPreviousMatchingSibling;
exports.getPreviousNonMatchingSibling = getPreviousNonMatchingSibling;
exports.getAbsoluteOffsets = getAbsoluteOffsets;
exports.getAbsoluteTopOffset = getAbsoluteTopOffset;
exports.getBoxOffsets = getBoxOffsets;
exports.getInnerText = getInnerText;
exports.isPointOverlayingObject = isPointOverlayingObject;
exports.boxesOverlap = boxesOverlap;
exports.boxesOverlapVertically = boxesOverlapVertically;
exports.boxesOverlapHorizontally = boxesOverlapHorizontally;
exports.getOuterHeight = getOuterHeight;
exports.getOuterWidth = getOuterWidth;
exports.getBufferHeight = getBufferHeight;
exports.getBufferWidth = getBufferWidth;
exports.getDimensionInPx = getDimensionInPx;
exports.getActualDimension = getActualDimension;
exports.purgeChildElements = purgeChildElements;
exports.purge = purge;
exports.getCaret = getCaret;
exports.isAlmostInView = isAlmostInView;
exports.getTouchedElement = getTouchedElement;
exports.centerLayer = centerLayer;
exports.centerElement = centerElement;
exports.parseFunc = parseFunc;
exports.cascadeElement = cascadeElement;
exports.fitObjectIntoScreen = fitObjectIntoScreen;
exports.focusOn = focusOn;
exports.selectAndFocusOn = selectAndFocusOn;
exports.renderHazeLayer = renderHazeLayer;
exports.renderOverlay = renderOverlay;
exports.removeOverlay = removeOverlay;
exports.reParent = reParent;
exports.getScrollLeftForAbsoluteParent = getScrollLeftForAbsoluteParent;
exports.getScrollTopForAbsoluteParent = getScrollTopForAbsoluteParent;
exports.renderTransparentIFrame = renderTransparentIFrame;
exports.pushOverlayObject = pushOverlayObject;
exports.pushTotalOverlayObject = pushTotalOverlayObject;
exports.pushOverlayObjectAtOffsets = pushOverlayObjectAtOffsets;
exports.popOverlayObject = popOverlayObject;
exports.popAllOverlayObjects = popAllOverlayObjects;
exports.disableSelection = disableSelection;
exports.disableSelectionWithoutCursorStyle = disableSelectionWithoutCursorStyle;
exports.enableSelection = enableSelection;
exports.setCursor = setCursor;
exports.cursorDefault = cursorDefault;
exports.cursorPointer = cursorPointer;
exports.cursorWait = cursorWait;
exports.getCellAtColumn = getCellAtColumn;
exports.getCellAtRow = getCellAtRow;
exports.copyTable = copyTable;
exports.copyTableJquery = copyTableJquery;
exports.clearTable = clearTable;
exports.getWindowWidth = getWindowWidth;
exports.getWindowHeight = getWindowHeight;
exports.getScrollLeft = getScrollLeft;
exports.getScrollTop = getScrollTop;
exports.scrollXToMiddle = scrollXToMiddle;
exports.launchNewWindow = launchNewWindow;
exports.getIFrameDocument = getIFrameDocument;
exports.getEvent = getEvent;
exports.popEvent = popEvent;
exports.triggerNativeEvent = triggerNativeEvent;
exports.relatedTargetInElementSubtree = relatedTargetInElementSubtree;
exports.cancelEventBubbling = cancelEventBubbling;
exports.cancelEventAndPreventDefault = cancelEventAndPreventDefault;
exports.enterKeyHit = enterKeyHit;
exports.whichKeyHit = whichKeyHit;
exports.isCtrlHeld = isCtrlHeld;
exports.isShiftHeld = isShiftHeld;
exports.getEventTarget = getEventTarget;
exports.isNavigationInput = isNavigationInput;
exports.verifyIsDigitInput = verifyIsDigitInput;
exports.verifyIsInputNumericDecimalSeparator = verifyIsInputNumericDecimalSeparator;
exports.numberOfDecimals = numberOfDecimals;
exports.macOSKeyboardRightClick = macOSKeyboardRightClick;
exports.macOSMouseRightClick = macOSMouseRightClick;
exports.isRightClick = isRightClick;
exports.isLeftClick = isLeftClick;
exports.isMetaHeld = isMetaHeld;
exports.trim = trim;
exports.startsWith = startsWith;
exports.getTextAfterUnderscore = getTextAfterUnderscore;
exports.getTextBeforeUnderscore = getTextBeforeUnderscore;
exports.extractNumber = extractNumber;
exports.getTextAfterSubstring = getTextAfterSubstring;
exports.getTextBeforeSubstring = getTextBeforeSubstring;
exports.removeTrailingSlash = removeTrailingSlash;
exports.removeTrailingPound = removeTrailingPound;
exports.removeChars = removeChars;
exports.replaceAll = replaceAll;
exports.jsHash = jsHash;
exports.textCounter = textCounter;
exports.saveAndClear = saveAndClear;
exports.escapeString = escapeString;
exports.formatString = formatString;
exports.removeWhitespacesFromTable = removeWhitespacesFromTable;
exports.splitText = splitText;
exports.redirectToUrl = redirectToUrl;
exports.checkURLProtocol = checkURLProtocol;
exports.getSymbolToAppendNextParam = getSymbolToAppendNextParam;
exports.restoreDefaultCursor = restoreDefaultCursor;
exports.encodeText = encodeText;
exports.decodeText = decodeText;
exports.constrain = constrain;
exports.deepClone = deepClone;
exports.cloneCustomAttributes = cloneCustomAttributes;
...
exports.toBlockNotation = toBlockNotation;
exports.getAsFunction = getAsFunction;
exports.toFunction = toFunction;
exports.BusyMonitor = BusyMonitor;
...
exports.unloadAllScripts = unloadAllScripts;
exports.loadCSSFiles = loadCSSFiles;
exports.loadCSSFile = loadCSSFile;
exports.unloadCSSFiles = unloadCSSFiles;
exports.unloadCSSFile = unloadCSSFile;
exports.createCSSRule = createCSSRule;
exports.getFirstMatch = getFirstMatch;
exports.dialogOnMouseDown = dialogOnMouseDown;
exports.cancelEventDrag = cancelEventDrag;
exports.encodeUriParameter = encodeUriParameter;
exports.replaceHTML = replaceHTML;
exports.gotoDefaultLocation = gotoDefaultLocation;
exports.gotoFolderExplorer = gotoFolderExplorer;
exports.switchDesign = switchDesign;
exports.showDiv = showDiv;
exports.hideDiv = hideDiv;
exports.borderImage = borderImage;
exports.resetRadio = resetRadio;
exports.launchAboutDlg = launchAboutDlg;
exports.hideAboutDlg = hideAboutDlg;
...
exports.JSCookie = JSCookie;
exports.sessionManager = sessionManager;
exports.buildActionUrl = buildActionUrl;
exports.pageDimmer = pageDimmer;
exports.fileSender = fileSender;
exports.ajaxIframeDownload = ajaxIframeDownload;
exports.parseAsDate = parseAsDate;
exports.parseAsDateTime = parseAsDateTime;
