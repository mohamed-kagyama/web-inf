...
var Truncator = Class.create({
  initialize: function initialize(elements, fixedWidth) {
  ...
  truncate: function truncate(element, columnWidth) {
  ...
  untruncate: function untruncate(elements) {
    elements.each(function (element) {
      if (element.nodeName == 'ABBR') {
  ...
  ellipsifyString: function ellipsifyString(s) {
    return s + '&hellip;';
  },
  truncateString: function truncateString(s, full) {
