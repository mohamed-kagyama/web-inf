...
module.exports = BaseDataSourceModel.extend({
  ...
  getCustomAttributeValuesFromConnectionUrl: function getCustomAttributeValuesFromConnectionUrl() {
    var foundParts,
        currentDriver = this.getCurrentDriver(); // the below line of code is added just for mysql old version datasource where allowPublicKeyRetrieval=true is not available '
    ...
    var url = this.get('connectionUrl'),
  ...
  addMissingProperties: function addMissingProperties(currentDriver, url) {
    var isAllowPublicKey = url && url.indexOf('allowPublicKeyRetrieval') == -1 && currentDriver.get('jdbcUrl').indexOf('allowPublicKeyRetrieval') != -1;
