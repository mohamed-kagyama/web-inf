...
var BaseDataSourceModel = require('./model/BaseDataSourceModel');
...
var history = require('../util/historyHelper');

var dataSourceViewFactory = require('./factory/dataSourceViewFactory');

var dataSourceResourceTypes = require('./enum/dataSourceResourceTypes');

var saveDialogViewFactory = require('./factory/saveDialogViewFactory');

var dataSourceMainTemplate = require("text!./template/dataSourceMainTemplate.htm");
...
var CustomDataSourcesCollection = require('./collection/CustomDataSourceCollection');
...
module.exports = Backbone.View.extend({
  ...
  constructor: function constructor(options) {
    var args = Array.prototype.slice.call(arguments, 0); // since the options object will be changed, we need to make the copy of it
  ...
  initialize: function initialize(options) {
    ...
    history.saveReferrer(this.historyBackToken); // Starting the game !
    ...
    if (this.options.resourceUri) {
      ...
      var self = this,
          modelToEdit = new BaseDataSourceModel({
        uri: this.options.resourceUri
      });
  ...
  renderDataSourceContainer: function renderDataSourceContainer() {
  ...
  onDataSourceTypeChange: function onDataSourceTypeChange(e) {
  ...
  _prepareSaveDialog: function _prepareSaveDialog(createDomainMode) {
  ...
  onSaveClick: function onSaveClick(e) {
    ...
    var self = this,
        ...
        funcOnceValidationPassed = function funcOnceValidationPassed() {
  ...
  _onSaveAndCreateDomainDone: function _onSaveAndCreateDomainDone(resourceModel) {
    redirectToUrl.redirect(CreateDomainLink({
      resourceUri: resourceModel.get('uri')
    }));
  },
  _onSaveFail: function _onSaveFail(model, xhr) {
    if (this.saveDialog) {
