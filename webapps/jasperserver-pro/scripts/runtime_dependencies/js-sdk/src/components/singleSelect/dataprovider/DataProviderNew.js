...
var PAGE_SIZE = 999,
    ...
    SERIAL_REQUESTS_DELAY = 50,
    //ms
CRITERIA_PARAMETER_NAME = "criteria",
    MAX_VALUE = 2147483647; //Equivalent to Java's Integer.MAX_VALUE

var DataProvider = function DataProvider(options) {
  _.bindAll(this, "getData");

  options = options || {}; //Holds cached data

  this.dataCache = []; //holds total data count

  this.dataCacheTotal = null; //holds information about cached pages in format:
  //[true, undefined, true]
  //means that there are two cached pages: 0-999, 2000-2999

  this.dataCachePages = []; //size of the page

  this.pageSize = options.pageSize || PAGE_SIZE; //name of the parameter used for searching by criteria

  this.criteriaKey = options.searchKey || CRITERIA_PARAMETER_NAME; //max count of cached searches

  this.maxSearchCacheSize = options.maxSearchCacheSize || SEARCH_CACHE_SIZE; //holds info about data providers for specific searches

  this.getDataByCriteriaHolder = {}; //whether to control when we need to get total

  this.controlGetTotal = typeof options.controlGetTotal !== "undefined" ? options.controlGetTotal : false; //optional dataconverter for data which cames through request

  this.dataConverter = options.dataConverter; //if true - last criteria will be saved untill it will be changed

  this.saveLastCriteria = typeof options.saveLastCriteria !== "undefined" ? options.saveLastCriteria : false; //saved criteria;

  this.lastCriteria = null; //Quick serial requests with same params will be delayed if > 0

  this.serialRequestsDelay = typeof options.serialRequestsDelay !== "undefined" ? options.serialRequestsDelay : SERIAL_REQUESTS_DELAY; //Object where promise for last sent request is stored until request will be done
  //uset to put all subsequent requests with same params to a queue

  this._setSerialRequestsDelayedPromise(null); //original data provider. by default it's AJAX request
...
_.extend(DataProvider.prototype, {
  ...
  /* Internal methods */
  _requestData: function _requestData(options, pagesToLoad, deferred) {
    ...
    if (this.serialRequestsDelay > 0) {
      ...
      this.deferredTimeout = setTimeout(_.bind(this._requestDataDeferred, this, options, optionsToQuery, deferred), this.serialRequestsDelay);
    } else {
      this._requestDataDeferred(options, optionsToQuery, deferred);
    }
  },
  _requestDataDeferred: function _requestDataDeferred(options, optionsToQuery, deferred) {
    var pagesToLoad = this._getPagesToLoad(options);
    ...
    if (!pagesToLoad) {
      ...
      if (this.serialRequestsDelayedPromise && _.isEqual(optionsToQuery, this.serialRequestsDelayedPromise.query)) {
        ...
        this.serialRequestsDelayedPromise.promise.done(deferred.resolve).fail(deferred.reject);
      } else {
        ...
        this.request(optionsToQuery).done(_.bind(this._requestDone, this, options, optionsToQuery, deferred)).fail(_.bind(this._requestFailed, this, deferred));
      }
    }
  },
  //Called when external request is done
  _requestDone: function _requestDone(options, optionsToQuery, deferred, data) {
    this._putDataToCache(optionsToQuery, data); //clear promise holder since request is done
  ...
  _requestFailed: function _requestFailed(deferred, response, status, xhr) {
    deferred.reject(response, status, xhr);

    this._setSerialRequestsDelayedPromise(null);
  },
  _setSerialRequestsDelayedPromise: function _setSerialRequestsDelayedPromise(delayedPromiseObject) {
    this.serialRequestsDelayedPromise = delayedPromiseObject;
  },
  _resolveDeferredUsingDataFromCache: function _resolveDeferredUsingDataFromCache(deferred, options) {
  ...
  _getPagesToLoad: function _getPagesToLoad(options) {
  ...
  _findPageIndex: function _findPageIndex(index) {
    return Math.floor(index / this.pageSize);
  },
  _convertLimitOffsetToFirstLast: function _convertLimitOffsetToFirstLast(options) {
    var first = options && options.offset ? options.offset : 0,
        last = options && options.limit ? first + options.limit - 1 : this.dataCacheTotal ? this.dataCacheTotal - 1 : this.maxLimit;
  ...
  _getDataFromCache: function _getDataFromCache(options) {
  ...
  _putDataToCache: function _putDataToCache(options, result) {
    var i;
    ...
    for (i = 0; i < result.data.length; i++) {
      this.dataCache[i + options.offset] = this.dataConverter ? this.dataConverter(result.data[i]) : result.data[i];
    }
   ...
   * @private
   */
  _getDataByCriteria: function _getDataByCriteria(criteria) {
   ...
   * @private
   */
  _createDataProviderForCriteria: function _createDataProviderForCriteria(criteria) {
