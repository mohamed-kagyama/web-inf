...
var makeStringRegex = function makeStringRegex(str) {
  return str == null ? '' : str.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
}; // Need to escape chars like ) and ( in order to construct correct regex later /(?: \)|\( )/g


var regexKeys = function regexKeys(map) {
  var arr = [];

  for (var k in map) {
    if (Object.prototype.hasOwnProperty.call(map, k)) arr.push(makeStringRegex(k));
  }
...
var unescapeMap = function () {
  ...
  for (k in escapeMap) {
    if (Object.prototype.hasOwnProperty.call(escapeMap, k)) map[escapeMap[k]] = k;
  }
...
var _isHTMLString = function _isHTMLString(str) {
  if (!(typeof str === 'string' || str instanceof String)) return false; // not an HTML string
...
var _stringCanonicObfuscator = function _stringCanonicObfuscator(str, obfChar) {
...
function _canonicalize(string) {
  string = string == null ? '' : string; // exclude potentially hard escaped chars from escapeMap values

  string = canonicExclusionRegex.test(string) ? string.replace(canonicExclusionRegex, function (match) {
    return canonicExclusionMap[match];
  }) : string;
  var canonicElem = document.createElement('textarea');
  canonicElem.innerHTML = string; // eslint-disable-line

  string = canonicElem.value; // revert the chars excluded above to their original values

  string = reverseCanonicExclusionRegex.test(string) ? string.replace(reverseCanonicExclusionRegex, function (match) {
    return reverseCanonicExclusionMap[match];
  }) : string;
  return string;
}
/**
...
function _getHtmlTagWhitelist() {
  ...
  if (configHtmlTagWhitelist.length > 0 && !_getHtmlTagWhitelist.whitelistInitialized) {
    htmlTagWhiteList = configHtmlTagWhitelist.startsWith("+") ? htmlTagWhiteList + ',' + configHtmlTagWhitelist.substr(1) : configHtmlTagWhitelist;
...
function _getWhitelistLeftRegex() {
  if (_getWhitelistLeftRegex.leftTagRegexp !== undefined) return _getWhitelistLeftRegex.leftTagRegexp;
...
function _getWhitelistRightRegex() {
  if (_getWhitelistRightRegex.rightTagRegexp !== undefined) return _getWhitelistRightRegex.rightTagRegexp;
...
var _defaultAttribSoftEscapeMap = {
  'regex': [/\bjavascript:/ig, /\bon(\w+?)\s*=/ig, /\bsrcdoc\s*=/ig],
  'replacement': ['', '&#111;&#110;$1=', '&#115;&#114;&#99;doc=']
};
...
function _getAttribSoftHtmlEscapeMap() {
  if (_getAttribSoftHtmlEscapeMap.attribSoftEscapeArr !== undefined) return _getAttribSoftHtmlEscapeMap.attribSoftEscapeArr;
...
var _xssSoftHtmlEscape = function _xssSoftHtmlEscape(string, options) {
  string = string == null ? '' : string;
  options = options || {};
  if (!_isHTMLString(string)) return string; // The case in which options.whitelist escape was applied first; string starts with nonce.
  ...
  if (noncePref && string.indexOf(noncePref) === 0) return string.substring(noncePref.length); // If the string contains nonce (not noncePrefix in the 1st pos-n), it comes from JRS; it's safe to exec javascript.

  var xssNonce = _getXssNonce();

  if (xssNonce && string.indexOf(xssNonce) > 0) return string;
  string = _canonicalize(string); //avoid escaping < or > in <TAG> or </TAG>, where TAG is white-listed
  ...
  if (options.whiteList && options.whiteList instanceof Array && options.whiteList.length > 0) {
    ...
    string = rtLeftTagRegexp.test(string) ? string.replace(rtLeftTagRegexp, '&lt;') : string; //escape </TAG>
    ...
    var tmpLeftTagRegexp = _getWhitelistLeftRegex(),
        tmpRightTagRegexp = _getWhitelistRightRegex();
    ...
    string = tmpLeftTagRegexp.test(string) ? string.replace(tmpLeftTagRegexp, '&lt;') : string; //escape </TAG>
...
var _xssHardEscape = function _xssHardEscape(string) {
  string = string == null ? '' : string;
  if (!(typeof string === 'string' || string instanceof String)) return string; // The case in which soft html escape with options.whitelist was applied first; string starts with nonce.
  // Remove nonce prefix.  Hard escape happens anyways.

  var noncePref = _getNoncePrefix();

  if (noncePref && string.indexOf(noncePref) === 0) string = string.substring(noncePref.length);
  hardEscapeRegex.lastIndex = 0;
  string = hardEscapeRegex.test(string) ? string.replace(hardEscapeRegex, function (match) {
...
var _xssUnescape = function _xssUnescape(string) {
  string = string == null ? '' : string;
  ...
  if (noncePref && string.indexOf(noncePref) === 0) string = string.substring(noncePref.length); // If the string contains nonce, it comes from JRS: was not escaped

  var xssNonce = _getXssNonce();

  if (xssNonce && string.indexOf(xssNonce) >= 0) return string;
  var unescapeRegexp = RegExp('(?:' + regexKeys(unescapeMap).join('|') + ')', 'ig');
  return unescapeRegexp.test(string) ? string.replace(unescapeRegexp, function (match) {
