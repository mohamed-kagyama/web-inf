...
module.exports = {
  ...
  isLogicFocusable: function isLogicFocusable(element) {
    ...
    var $el = $(element),
        ...
        tabindex = $el.attr('tabindex'); // These elements are always focusable (...unless you're in Safari
  ...
  // normally focusable (though the table itself will be if the table
  // element specifies a tabindex).
  isUserFocusable: function isUserFocusable(element) {
    if (this.nullOrUndefined(element)) {
    ...
    var el = $(element),
        canFocus = false,
        nodeName = el.prop('nodeName'),
        tabindex = el.attr('tabindex'); // These elements are always focusable...
  ...
  suspendFocusability: function suspendFocusability(element) {
    if (this.nullOrUndefined(element)) {
  ...
  resumeFocusability: function resumeFocusability(element) {
    if (this.nullOrUndefined(element)) {
  ...
  // children.
  ensureFocusabilityBeyond: function ensureFocusabilityBeyond(element) {
  ...
  //
  forceFocus: function forceFocus(element) {
    logger.debug("stdnav.forceFocus(" + element.nodeName + "#" + element.id + ")");
    var $el = $(element); // See if we should use a tabindex of zero, or something else.  Keep in mind
  ...
  //
  setSuperfocus: function setSuperfocus(element, fireCallbacks) {
    ...
    if (fireCallbacks) {
      ...
      this._prepSubfocusIn(element); // Call the subfocusin action on the new subfocus.
  ...
  setSubfocus: function setSubfocus(element, fireCallbacks) {
  ...
  _onFocusIn: function _onFocusIn(ev) {
    ...
    this.updateDebugInfo(); // Hack for IE11, force a potential visual change
    ...
    var eventFocus = ev.target; // Figure out what element should actually have focus.  For example,
    ...
    if (fixedFocus !== eventFocus) {
      ...
      this._refocusing = true; // (NOTE: Do NOT call _unforceFocus for the initial target.)

      this.forceFocus(fixedFocus); // No further processing should occur.
    } else {
      // This is our final focus target.
      this._refocusing = false; // Was there a net change?
      ...
      if (eventFocus === this._priorFocus) {
        ...
        if (this._priorFocus) {
          this.runAction('focusout', this._priorFocus); // If the control was temporarily made focusable, undo that.

          this._unforceFocus(this._priorFocus);
        } // Latch the new focus.


        this._currentFocus = eventFocus; // Figure out the new superfocus, partly so that we can tell if the
        ...
        if (this._currentSuperfocus !== this._priorSuperfocus) {
          ...
          $(this._currentSuperfocus).addClass('superfocus'); // This ensures SHIFT-TAB will properly move to the prior region,
        ...
        this.runAction('subfocusin', this._currentSubfocus); // Ensure focus can be moved to other areas in the DOM.
  ...
  _prepSubfocusIn: function _prepSubfocusIn(element) {
  ...
  _unforceFocus: function _unforceFocus(element) {
    var $el = $(element);
    var priorTabindex = $el.data('prior-tabindex');

    if (this.nullOrUndefined(priorTabindex)) {// forceFocus does not seem to have been called, was not needed, or
  ...
  _onFocusOut: function _onFocusOut(ev) {
    this.updateDebugInfo();

    if (this._refocusing === true) {// Intermediate target-- do not update latches.
    } else {
      if ($('.superfocus').length > 0) {
    ...
    var MAIN_NAVIGATION_EXIT_POINT = "#about a.superfocus.subfocus"; // Remove focus classes when leaving the global navigation entry-point and exit-point.
