...
var DataProvider = function DataProvider(options) {
  ...
  this.getDataByCriteriaHolder = {}; //holds criteria value

  this.criteriaValuesCache = []; //whether to control when we need to get total
  ...
  this.serialRequestsDelayedPromise = null; //original data provider. by default it's AJAX request
...
_.extend(DataProvider.prototype, {
  ...
  getData: function getData(options) {
    ...
    if (!_.isEmpty(this.criteriaValuesCache)) {
      if (typeof options[this.criteriaKey] === "undefined") {
        options[this.criteriaKey] = _.first(this.criteriaValuesCache);
      }
    }
  ...
  _requestDataDeferred: function _requestDataDeferred(options, optionsToQuery, deferred) {
    ...
    if (!pagesToLoad) {
      ...
      if (this.serialRequestsDelayedPromise && _.isEqual(optionsToQuery, this.serialRequestsDelayedPromise.query)) {
        ...
        this.request(optionsToQuery).done(_.bind(this._requestDone, this, options, optionsToQuery, deferred)).fail(deferred.reject);
      }
    }
  },
  ...
  _convertLimitOffsetToFirstLast: function _convertLimitOffsetToFirstLast(options) {
    var first = options && options.offset ? options.offset : 0,
        last = options && options.limit ? first + options.limit - 1 : this.dataCacheTotal ? this.dataCacheTotal - 1 : MAX_VALUE;
