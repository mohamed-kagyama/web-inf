...
var TreeLevel = require('./TreeLevel');

var TreeDataLayer = require('./TreeDataLayer');

var listTemplate = require("text!./template/treeLevelListTemplate.htm");

var levelTemplate = require("text!./template/treeLevelTemplate.htm");
...
var Tree = Backbone.View.extend({
  ...
  initialize: function initialize(options) {
    this.rootLevel = new TreeLevel(_.extend({}, this._options, {
      levelHeight: options.rootLevelHeight
    }, {
      el: this.$('li')[0]
    }, {
      item: {
  ...
  expand: function expand(nodeId, options) {
    var level = this.getLevel(nodeId);
    level && level.open(options);
    return this;
  },
  collapse: function collapse(nodeId, options) {
    var level = this.getLevel(nodeId);
    level && level.close(options);
    return this;
  },
  _getNodeByLevelId: function _getNodeByLevelId(levelId) {
  ...
  select: function select(levelId) {
  ...
  getDataLayer: function getDataLayer(level) {
    return this.customDataLayers && this.customDataLayers[level.id] ? this.customDataLayers[level.id] : this.defaultDataLayer;
  },
  _addItem: function _addItem(childItems, parentId, itemData) {
    !_.findWhere(childItems, {
      id: itemData.id
    }) && childItems.push(itemData);
    return childItems;
  },
  _updateItem: function _updateItem(childItems, itemData) {
  ...
  _getNodeItems: function _getNodeItems(parentId, level, itemData, action) {
  ...
  _selectTreeNode: function _selectTreeNode(pathToSelect, $scrollContainer) {
  ...
  _openPath: function _openPath(path, dfd, index) {
    if (!path) {
      return dfd.resolve();
    }
  ...
  getLevelId: function getLevelId(repositoryPath) {
    var levelId = '';
...
function addLevels(parentLevel) {
  var items = parentLevel.list.model.get('items'),
      self = this;
  items = _.where(items, {
    _node: true
  });
  ...
  }).length && parentLevel.$('.j-view-port-chunk').css({
    height: 'auto'
  });
}
