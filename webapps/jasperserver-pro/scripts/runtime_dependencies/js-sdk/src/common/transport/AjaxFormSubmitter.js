...
function parseResponse(document) {
  ...
  if (document.firstChild.innerText && document.firstChild.innerText !== "") {
    if (document.body) {
      ...
      result.async = false; //remove injected by IE symbols
  ...
  return parseChildren(result.firstChild.nodeName.toLowerCase() === "xml" ? result.firstChild.nextSibling : result.firstChild);
}
...
function ajaxUpload(form) {
  ...
  form.submit(function (e) {
    ...
    request({
      ...
      error = error.responseJSON || error || {}; // default error code
...
function iframeUpload(form, name) {
  ...
  iframe.on("load", function () {
    try {
      ...
      result.reject({
        errorCode: "error.invalid.response"
      });
    }
...
var AjaxUploader = function AjaxUploader(uploadForm, url, method, enctype) {
