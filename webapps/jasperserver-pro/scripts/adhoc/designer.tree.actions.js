...
adhocDesigner.copyParentStructure = function (sourceNode, destTree) {
  if (sourceNode && destTree) {
    ...
    var metanodeBuilder = window.localContext.metaNodeBuilder ? window.localContext.metaNodeBuilder : adhocDesigner.metaNodeBuilder;
...
adhocDesigner.getNodeByEvent = function (event) {
  return this.measuresTree.getTreeNodeByEvent(event) || this.dimensionsTree.getTreeNodeByEvent(event);
};
