  ...
  return function (inputOptions) {
    var defaultOptions = {
      ...
      saveHandler: function saveHandler(placeToSave) {
        ...
        //use this to delay dialog close until server confirms successful save
    ...
    this.open = function (initialPlaceToSave) {
      ...
      // max two tree supported on dialog
    ...
    this.close = function () {
      dialogElement.find(opt.okButtonMatcher).off("click", okButtonHandler);
      dialogElement.find(opt.cancelButtonMatcher).off("click", cancelButtonHandler);
...
JRS.RepositorySelectionDialog = function (jQuery, dynamicTree) {
  var defaultOptions = {
    ...
    acceptOnlyLeaf: true,
    okHandler: function okHandler(selectedUri) {
      //default ok handler
      var deferred = jQuery.Deferred(); //use this to delay dialog close until server confirms successful save
      //use this to delay dialog close until server confirms successful save
  ...
  return function (inputOptions) {
    ...
    this.open = function () {
      ...
      function processTreeNodeIcon(node) {
        //TODO most code copypasted from adhoc.start.js. Think of reusing
        if (node.isParent()) {
...
JRS.generatorSelect = function ($, _, jrsConf) {
  ...
  var Control = function Control(options) {
    ...
    this.state = 'DEFAULT'; // internally we maintain three arrays for radios, inputs and states (abstraction over available options)
  ...
  */
  ...
  function registerGeneratorOptions(instance, values) {
    ...
    if (values && values.length > 0) {
      ...
      _.each(values, function (value) {
        select.append($('<option></option>').val(value.id).text(value.label));
      });
    } else {
  ...
  function registerEvents(instance) {
    instance.template.on('click', function (evt) {
      if (isRadio(evt.target)) {
        ...
        // issue JRS-15947: browser button is active while it has 'disabled' state
        if (isButtonEnabled(evt.target)) {
  ...
  function isRadio(element) {
    return element.type ? element.type === 'radio' // if element is radio input
    : // if element is radio input
    element.control && element.control.type == 'radio'; // if element is label of radio
  }

  function isButton(element) {
    return element.type && element.type == 'button' || element.tagName === 'SPAN' && element.parentNode.type == 'button';
  }
  ...
  function isButtonEnabled(element) {
    ...
    return button[0].disabled === false;
  }
  ...
  function browseHandler(instance) {
    ...
    if (!instance._dialog) {
      instance._dialog = new JRS.TemplateDialog({
        ...
        okHandler: function okHandler(uri) {
...
var RepositorySelectionDialog = JRS.RepositorySelectionDialog;
var ViewQueryDialog = JRS.ViewQueryDialog;
var SaveAsDialog = JRS.SaveAsDialog;
var GeneratorPropertiesDialog = JRS.GeneratorPropertiesDialog;
var generatorSelect = JRS.generatorSelect;
var TemplateDialog = JRS.TemplateDialog;
exports.RepositorySelectionDialog = RepositorySelectionDialog;
exports.ViewQueryDialog = ViewQueryDialog;
exports.SaveAsDialog = SaveAsDialog;
exports.GeneratorPropertiesDialog = GeneratorPropertiesDialog;
