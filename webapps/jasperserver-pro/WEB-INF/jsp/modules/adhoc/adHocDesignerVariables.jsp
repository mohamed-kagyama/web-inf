...
<c:choose>
    <c:when test="${not empty requestScope.aruLabel}">
        <c:set var="pageHeaderText" value="${requestScope.aruLabel}"/>
    </c:when>
    <c:otherwise>
        <c:set var="pageHeaderText">
            <spring:message code="ADH_002c_CANVAS" javaScriptEscape="true"/>
        </c:set>
    </c:otherwise>
</c:choose>
